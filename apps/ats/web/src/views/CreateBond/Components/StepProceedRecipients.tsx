...
export const StepProceedRecipients = () => {
  ...
  const proceedRecipients: IProceedRecipient[] =
    currentProceedRecipientsIds.map((address, index) => ({
      address,
      data: currentProceedRecipientsData[index] || '',
    }));

  const handleAddProceedRecipient = handleLocalSubmit((data) => {
    const newProceedRecipientsIds = [
      ...currentProceedRecipientsIds,
      data.address.trim(),
    ];
    const newProceedRecipientsData = [
      ...currentProceedRecipientsData,
      data?.data?.trim() ?? '',
    ];
  ...
  const handleRemoveProceedRecipient = (addressToRemove: string) => {
    ...
    if (indexToRemove !== -1) {
      const newProceedRecipientsIds = currentProceedRecipientsIds.filter(
        (_, index) => index !== indexToRemove,
      );
      const newProceedRecipientsData = currentProceedRecipientsData.filter(
        (_, index) => index !== indexToRemove,
      );
    ...
    <FormStepContainer>
      <Stack gap={2}>
        <Text textStyle="HeadingMediumLG">
          {t('stepProceedRecipients.title')}
        </Text>
        <Text textStyle="BodyTextRegularMD">
          {t('stepProceedRecipients.subtitle')}
        </Text>
      ...
      <Stack w="full">
        <Text textStyle="BodyTextRegularSM">
          {t('stepProceedRecipients.address')}
        </Text>
        <InputController
          control={localControl}
          id="address"
          ...
          rules={{
            ...
            validate: (value: string) =>
              ...
              t('stepProceedRecipients.invalidHederaId'),
          }}
        />
      </Stack>
      <Stack w="full">
      ...
      <Stack w="full" align="end" justifyContent={'flex-end'}>
        <Button
          onClick={handleAddProceedRecipient}
          size="md"
