...
export const SSIManager = () => {
  ...
  const { data: issuersList, isLoading: isLoadingIssuersList } =
    useGetIssuersList(
  ...
  const hasSSIManagerRole = useMemo(
    () =>
      accountRoles.findIndex(
        (rol) => rol === SecurityRole._SSI_MANAGER_ROLE,
      ) !== -1,
    [accountRoles],
  );
  ...
  return (
    <>
      ...
      >
        ...
        <Table
          name="ssi-manager-list"
          columns={columns}
          data={issuersList ?? []}
          isLoading={isLoadingIssuersList}
        />
      </Stack>
      <SSIManagerModal isOpen={isOpen} onClose={onClose} />
      <SSIManagerRevocationModal
        isOpen={isOpenRevocationModal}
        onClose={onCloseRevocationModal}
      />
      <PopUp
