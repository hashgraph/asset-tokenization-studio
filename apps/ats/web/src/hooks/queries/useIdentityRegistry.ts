...
export const GET_IDENTITY_REGISTRY = (securityId: string) =>
  `GET_IDENTITY_REGISTRY_${securityId}`;
...
export const useGetIdentityRegistry = <TError, TData = string>(
  ...
  return useQuery(
    [GET_IDENTITY_REGISTRY(request.securityId)],
    () => SDKService.getIdentityRegistry(request),
    options,
  );
};
