...
import {
  GET_ISSUERS_LIST,
  GET_REVOCATION_REGISTRY_ADDRESS,
} from '../queries/useSSIManager';
...
export const useRemoveIssuer = () => {
  ...
  return useMutation(
    (req: RemoveIssuerRequest) => SDKService.removeIssuer(req),
    {
      onSuccess(data, variables) {
        queryClient.invalidateQueries({
          queryKey: [GET_ISSUERS_LIST(variables.securityId)],
...
export const useSetRevocationRegistryAddress = () => {
  ...
  return useMutation(
    (req: SetRevocationRegistryAddressRequest) =>
      SDKService.setRevocationRegistryAddress(req),
    {
      onSuccess(data, variables) {
        queryClient.invalidateQueries({
