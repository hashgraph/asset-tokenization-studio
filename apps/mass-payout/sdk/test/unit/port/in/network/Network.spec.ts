...
import {
  ...
  mainnet,
  unrecognized,
} from "@domain/network/Environment"
...
describe("Network", () => {
  ...
  beforeEach(() => {
    ...
    transactionService = {
      getHandler: jest
        .fn()
        .mockReturnValue({ stop: jest.fn().mockResolvedValue(true) }),
  ...
  describe("executeConnection", () => {
    it("should call handler.register and return ExecuteConnectionResponse", async () => {
      ...
      const res = await service.executeConnection(
        testnet,
        SupportedWallets.DFNS,
        { id: "acc" } as any
      )

      expect(transactionService.getHandlerClass).toHaveBeenCalledWith(
        SupportedWallets.DFNS
      )
  ...
  describe("connect", () => {
    it("should validate, set network, and call executeConnection", async () => {
      ...
      jest
        .spyOn(service, "executeConnection")
        .mockResolvedValue(execRes as any)
      ...
      expect(service.executeConnection).toHaveBeenCalledWith(
        req.network,
        req.wallet,
        undefined,
        undefined
