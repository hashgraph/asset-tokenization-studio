...
import { getSelector } from "@lib/operations";

const AMOUNT = 1000;
const DATA = "0x1234";
const EMPTY_VC_ID = EMPTY_STRING;

export function complianceTests(getCtx: () => AssetMockCtx): void {
  describe("Compliance Tests", () => {
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN setCompliance THEN transaction fails with Deactivated", async () => {
        await expect(asset.connect(signer_A).setCompliance(ethers.ZeroAddress)).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
    ...
    describe("Compliance notifications when compliance is address(0)", () => {
      ...
      it("GIVEN compliance set to address(0) WHEN redeemByPartition on a non-default partition THEN the call does not revert", async () => {
        ...
        await expect(asset.connect(signer_E).redeemByPartition(NON_DEFAULT_PARTITION, AMOUNT / 2, EMPTY_HEX_BYTES)).to
          .not.be.reverted;
      });
    ...
    describe("initializeCompliance", () => {
      it("GIVEN caller without DEFAULT_ADMIN_ROLE WHEN initializeCompliance is called THEN AccountHasNoRole", async () => {
        await expect(asset.connect(unknownSigner).initializeCompliance(ethers.ZeroAddress))
          .to.be.revertedWithCustomError(asset, "AccountHasNoRole")
