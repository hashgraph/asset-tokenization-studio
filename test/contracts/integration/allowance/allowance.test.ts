...
import { ADDRESS_ZERO, ATS_ROLES, DEFAULT_PARTITION, EMPTY_STRING, ZERO, RESOLVER_KEYS } from "@lib";
...
export function allowanceTests(getCtx: () => AssetMockCtx): void {
  describe("Allowance Facet Tests", () => {
    ...
    describe("Single partition", () => {
      let assetSignerC: IAssetMock;
      let assetSignerD: IAssetMock;
      ...
      beforeEach(async () => {
        ...
        assetSignerC = asset.connect(signer_C) as unknown as IAssetMock;
        assetSignerD = asset.connect(signer_D) as unknown as IAssetMock;
      ...
      describe("interop", () => {
        it("callers resolving through the diamond via AllowanceFacet still see the same state as via IAsset", async () => {
          const viaAllowance = await ethers.getContractAt("AllowanceFacet", await asset.getAddress(), signer_C);
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN approve THEN transaction fails with Deactivated", async () => {
        await expect(asset.approve(ethers.ZeroAddress, 0)).to.be.revertedWithCustomError(asset, "Deactivated");
      });

      it("GIVEN a deactivated asset WHEN increaseAllowance THEN transaction fails with Deactivated", async () => {
        await expect(asset.increaseAllowance(ethers.ZeroAddress, 0)).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
      ...
      it("GIVEN a deactivated asset WHEN decreaseAllowance THEN transaction fails with Deactivated", async () => {
        await expect(asset.decreaseAllowance(ethers.ZeroAddress, 0)).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
