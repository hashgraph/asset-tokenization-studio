...
import { RESOLVER_KEYS } from "@lib";
import { executeRbac } from "@test";
import { ATS_ROLES, DEFAULT_PARTITION } from "@lib";

const amount = 1000;

export function erc20VotesTests(getCtx: () => AssetMockCtx): void {
  describe("ERC20Votes Tests", () => {
    ...
    beforeEach(async () => {
      ...
      await asset.forceErc20VotesActivated(true);
    ...
    describe("Initialization", () => {
      ...
      it("GIVEN ERC20Votes not activated WHEN calling isActivated THEN returns false", async () => {
        await asset.forceErc20VotesActivated(false);
        const isActivated = await asset.isActivated();
    ...
    describe("Voting Power on Balance-Movement Paths", () => {
      ...
      it("GIVEN frozen tokens with token holder delegating WHEN unfreezePartialTokens THEN voting power is unchanged (same-holder no-op)", async () => {
        ...
        // so switch to single-partition on the outer asset.
        ...
        await asset.connect(signer_A).freezePartialTokens(signer_A.address, FREEZE_AMOUNT);

        const votesB_before = await asset.getVotes(signer_B.address);

        await expect(asset.connect(signer_A).unfreezePartialTokens(signer_A.address, FREEZE_AMOUNT)).to.not.emit(
          asset,
          "DelegateVotesChanged",
        );
    ...
    describe("Deactivated", () => {
      it("GIVEN a deactivated asset WHEN delegate THEN transaction fails with Deactivated", async () => {
        ...
        await expect(asset.connect(signer_A).delegate(ethers.ZeroAddress)).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
