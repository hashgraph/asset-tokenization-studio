...
import { ADDRESS_ZERO, ATS_ROLES, dateToUnixTimestamp, EMPTY_HEX_BYTES, EMPTY_STRING, ZERO, RESOLVER_KEYS } from "@lib";
...
export function clearingTests(getCtx: () => AssetMockCtx): void {
  describe("Clearing Tests", () => {
    ...
    async function singlePartitionSetup() {
    ...
    async function multiPartitionSetup() {
    ...
    describe("Single Partition", async () => {
      ...
      describe("Not in clearing mode", () => {
        it("GIVEN a token not in clearing mode WHEN create clearing THEN transaction fails with ClearingIsDisabled", async () => {
          ...
          await expect(
            asset.clearingRedeemFromByPartition(clearingOperationFrom, _AMOUNT),
          ).to.be.revertedWithCustomError(asset, "ClearingIsDisabled");
          await expect(
      ...
      describe("Clearing with zero and minimal amounts", () => {
        ...
        it("GIVEN a Token WHEN creating clearing redeem with amount 1 THEN transaction succeeds", async () => {
          await expect(asset.clearingRedeemByPartition(clearingOperation, 1)).to.emit(
            asset,
      ...
      describe("Control List", () => {
        ...
        it("GIVEN a blacklisted origin account WHEN approveClearingOperationByPartition with operation type Transfer THEN transaction fails with AccountIsBlocked", async () => {
          ...
          await expect(
            asset.approveClearingOperationByPartition(clearingIdentifierFromB),
          ).to.be.revertedWithCustomError(asset, "AccountIsBlocked");
      ...
      describe("Create clearing with wrong input arguments", () => {
        it("GIVEN a Token WHEN creating clearing with amount bigger than balance THEN transaction fails with InsufficientBalance", async () => {
          ...
          await expect(
            asset.clearingCreateHoldByPartition(clearingOperation, hold_wrong),
          ).to.be.revertedWithCustomError(asset, "InsufficientBalance");
          await expect(
        ...
        it("GIVEN a Token WHEN creating clearing passing wrong expirationTimestamp THEN transaction fails with WrongExpirationTimestamp", async () => {
          ...
          // Holds
          await expect(
            asset.clearingCreateHoldByPartition(clearingOperation__wrong, hold),
          ).to.be.revertedWithCustomError(asset, "WrongExpirationTimestamp");
          await asset.connect(signer_B).increaseAllowance(signer_A.address, _AMOUNT);
          await expect(
            asset.clearingCreateHoldFromByPartition(clearingOperationFrom_wrong, hold),
          ...
          // Redeems
          await expect(
            asset.clearingRedeemByPartition(clearingOperation__wrong, _AMOUNT),
          ).to.be.revertedWithCustomError(asset, "WrongExpirationTimestamp");
          await expect(
            asset.clearingRedeemFromByPartition(clearingOperationFrom_wrong, _AMOUNT),
          ).to.be.revertedWithCustomError(asset, "WrongExpirationTimestamp");
      ...
      describe("Manage clearing with wrong input arguments", () => {
        ...
        it("GIVEN a clearing transfer WHEN approveClearingOperationByPartition with wrong partition THEN transaction fails with PartitionNotAllowedInSinglePartitionMode", async () => {
          ...
          await expect(
            asset.approveClearingOperationByPartition(wrongClearingIdentifier),
          ).to.be.revertedWithCustomError(asset, "PartitionNotAllowedInSinglePartitionMode");
        });
      ...
      describe("Balance Adjustments", () => {
        ...
        it("GIVEN a clearing WHEN adjustBalances THEN clearing amount gets updated succeeds", async () => {
          ...
          expect(cleared_TotalAmount_After).to.be.equal(
            cleared_TotalAmount_Before * BigInt(adjustFactor * adjustFactor),
          );
          ...
          expect(balance_After).to.be.equal(
            (balance_Before - BigInt(9 * _AMOUNT)) * BigInt(adjustFactor * adjustFactor),
          );
        ...
        it("GIVEN a clearing WHEN adjustBalances THEN approve succeed", async () => {
          ...
          const balance_After_Approve_Partition_1_A = await asset.balanceOfByPartition(
            _PARTITION_ID_1,
            signer_A.address,
          );
          const balance_After_Approve_C = await asset.balanceOf(signer_C.address);
          const balance_After_Approve_Partition_1_C = await asset.balanceOfByPartition(
            _PARTITION_ID_1,
            signer_C.address,
          );
          ...
          expect(cleared_Amount_After).to.be.equal(
            (cleared_Amount_Before - BigInt(9 * _AMOUNT)) * BigInt(adjustFactor),
          );
        ...
        it("GIVEN a clearing WHEN adjustBalances THEN cancel succeed", async () => {
          ...
          const balance_After_Cancel_Partition_1_A = await asset.balanceOfByPartition(
            _PARTITION_ID_1,
            signer_A.address,
          );
          const balance_After_Cancel_C = await asset.balanceOf(signer_C.address);
          const balance_After_Cancel_Partition_1_C = await asset.balanceOfByPartition(
            _PARTITION_ID_1,
            signer_C.address,
          );
          const cleared_Amount_After = await asset.getClearedAmountFor(signer_A.address);
        ...
        it("GIVEN a hold WHEN adjustBalances THEN clearing succeeds", async () => {
          ...
          const balance_After_Clearing_Partition_1 = await asset.balanceOfByPartition(
            _PARTITION_ID_1,
            signer_A.address,
          );
          const cleared_Amount_After = await asset.getClearedAmountFor(signer_A.address);
    ...
    describe("Common Modifiers", () => {
      ...
      describe("initializeClearing", () => {
        ...
        it("GIVEN a caller without DEFAULT_ADMIN_ROLE WHEN initializeClearing is called THEN it reverts with AccountHasNoRole", async () => {
          await expect(asset.connect(signer_D).initializeClearing(true)).to.be.revertedWithCustomError(
            asset,
            "AccountHasNoRole",
          );
        });
        ...
        it("GIVEN a fresh deployment WHEN initializeClearing is called THEN emits ClearingInitialized", async () => {
          await asset.forceFacetNotRegistered(RESOLVER_KEYS.clearing);
      ...
      describe("onlyDefaultPartitionWithSinglePartition modifier", () => {
        it("GIVEN non-default partition WHEN calling cancelClearingOperationByPartition THEN transaction fails with PartitionNotAllowedInSinglePartitionMode", async () => {
          ...
          await expect(
            asset.cancelClearingOperationByPartition(wrongPartitionIdentifier),
          ).to.be.revertedWithCustomError(asset, "PartitionNotAllowedInSinglePartitionMode");
        });
        ...
        it("GIVEN non-default partition WHEN calling reclaimClearingOperationByPartition THEN transaction fails with PartitionNotAllowedInSinglePartitionMode", async () => {
          ...
          await expect(
            asset.reclaimClearingOperationByPartition(wrongPartitionIdentifier),
          ).to.be.revertedWithCustomError(asset, "PartitionNotAllowedInSinglePartitionMode");
        });
      });
      ...
      describe("Protected Clearing Operations", () => {
        ...
        beforeEach(async () => {
          ...
          domain = {
            ...
            version: "1.0.0",
            chainId: chainId,
            verifyingContract: asset.target.toString(),
          };
        ...
        describe("Modifier Tests", () => {
          ...
          describe("protectedClearingRedeemByPartition", () => {
            ...
            it("SHOULD revert WHEN clearing not activated (onlyClearingActivated modifier)", async () => {
              ...
              await expect(
                asset.connect(signer_A).protectedClearingRedeemByPartition(protectedClearingOp, _AMOUNT, sig),
              ).to.be.reverted;
            });
          });
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN activateClearing THEN transaction fails with Deactivated", async () => {
        await expect(asset.connect(signer_A).activateClearing()).to.be.revertedWithCustomError(asset, "Deactivated");
      });

      it("GIVEN a deactivated asset WHEN deactivateClearing THEN transaction fails with Deactivated", async () => {
