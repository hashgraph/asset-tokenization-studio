...
import { DEFAULT_PARTITION, ATS_ROLES, ZERO, dateToUnixTimestamp, RESOLVER_KEYS } from "@lib";
...
export function adjustBalancesFacetTests(getCtx: () => AssetMockCtx): void {
  describe("AdjustBalancesFacet Tests", () => {
    ...
    describe("Balance adjustments", () => {
      ...
      it("GIVEN balance adjustment created WHEN trying to get balance adjustment with wrong ID type THEN transaction fails with WrongIndexForAction", async () => {
        ...
        await expect(asset.getScheduledBalanceAdjustment(2)).to.be.revertedWithCustomError(
          asset,
          "WrongIndexForAction",
        );
      });
      ...
      describe("Force Cancel Scheduled Balance Adjustment", () => {
        ...
        it("GIVEN no existing balance adjustment WHEN forceCancelScheduledBalanceAdjustment with invalid ID THEN transaction fails with WrongIndexForAction", async () => {
          ...
          await expect(
            asset.connect(signer_C).forceCancelScheduledBalanceAdjustment(999),
    ...
    describe("getScheduledBalanceAdjustment", () => {
      it("GIVEN id is zero WHEN getScheduledBalanceAdjustment THEN reverts with WrongIndexForAction", async () => {
        await expect(asset.getScheduledBalanceAdjustment(0)).to.be.revertedWithCustomError(
          asset,
          "ZeroValueNotAllowed",
        );
      });
    ...
    describe("adjustBalances", () => {
      ...
      it("GIVEN an account without adjustBalances role WHEN adjustBalances THEN transaction fails with AccountHasNoRole", async () => {
        await expect(
          asset.connect(signer_C).adjustBalances(adjustFactor, adjustDecimals),
        ).to.be.revertedWithCustomError(asset, "AccountHasNoRole");
      });
      ...
      it("GIVEN an overflowing total supply WHEN adjustBalances THEN transaction fails with TotalSupplyOverflow", async () => {
        ...
        await expect(
          asset
            .connect(signer_A)
            .adjustBalances(BigInt(MAX_UINT256) / BigInt(TOTAL_SUPPLY) + BigInt(1), adjustDecimals),
        ).to.be.revertedWithCustomError(asset, "TotalSupplyOverflow");
      });
    });
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN adjustBalances THEN transaction fails with Deactivated", async () => {
        await expect(asset.adjustBalances(1, 0)).to.be.revertedWithCustomError(asset, "Deactivated");
      });

      it("GIVEN a deactivated asset WHEN triggerAndSyncAll THEN transaction fails with Deactivated", async () => {
        await expect(
          asset.triggerAndSyncAll(ethers.ZeroHash, ethers.ZeroAddress, ethers.ZeroAddress),
      ...
      it("GIVEN a deactivated asset WHEN forceCancelScheduledBalanceAdjustment THEN transaction fails with Deactivated", async () => {
        await expect(asset.forceCancelScheduledBalanceAdjustment(0)).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
