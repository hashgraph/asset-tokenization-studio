...
export function pauseTests(getCtx: () => AssetMockCtx): void {
  describe("Pause Tests", () => {
    ...
    describe("External Pause", () => {
      ...
      beforeEach(async () => {
        externalPauseMock = await (await ethers.getContractFactory("MockedExternalPause", deployer)).deploy();
        ...
        await asset.connect(deployer).addExternalPause(externalPauseMock.target);
      });
