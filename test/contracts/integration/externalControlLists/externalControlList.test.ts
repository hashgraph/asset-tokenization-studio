...
import { ADDRESS_ZERO, ATS_ROLES, DEFAULT_PARTITION, RESOLVER_KEYS } from "@lib";
...
export function externalControlListTests(getCtx: () => AssetMockCtx): void {
  describe("ExternalControlList Management Tests", () => {
    ...
    beforeEach(async () => {
      ...
      initMock1 = ctx.externalMocks.whitelist[0];
      initMock2 = ctx.externalMocks.blacklist[0];
      externalWhitelistMock1 = ctx.externalMocks.whitelist[1];
      externalBlacklistMock1 = ctx.externalMocks.blacklist[1];
      externalWhitelistMock2 = ctx.externalMocks.whitelist[2];
      ...
      await asset.connect(signer_A).addExternalControlList(initMock1.target as string);
      await asset.connect(signer_A).addExternalControlList(initMock2.target as string);
    ...
    describe("Add Tests", () => {
      it("GIVEN an unlisted external control list WHEN added THEN it is listed and event is emitted", async () => {
        ...
        await expect(asset.connect(signer_A).addExternalControlList(newControlList))
      ...
      it("GIVEN an invalid address WHEN adding it THEN it reverts with ZeroAddressNotAllowed", async () => {
        await expect(asset.connect(signer_A).addExternalControlList(ADDRESS_ZERO)).to.be.revertedWithCustomError(
          asset,
    ...
    describe("Remove Tests", () => {
      it("GIVEN a listed external control list WHEN removed THEN it is unlisted and event is emitted", async () => {
        ...
        const initialCount = await asset.getExternalControlListsCount();
        expect(initialCount).to.equal(4);
        await expect(asset.connect(signer_A).removeExternalControlList(controlListToRemove))
      ...
      it("GIVEN an unlisted external control list WHEN removing THEN it reverts with UnlistedControlList", async () => {
        ...
        await expect(asset.connect(signer_A).removeExternalControlList(randomAddress)).to.be.revertedWithCustomError(
          asset,
          "UnlistedControlList",
        );
      });
    ...
    describe("Update Tests", () => {
      ...
      it("GIVEN multiple external control lists WHEN updated THEN their statuses are updated and event is emitted", async () => {
        ...
        await expect(asset.connect(signer_A).updateExternalControlLists(controlListsToUpdate, activesToUpdate))
      ...
      it("GIVEN empty arrays WHEN updating THEN it succeeds and emits event", async () => {
        ...
        await expect(asset.connect(signer_A).updateExternalControlLists(controlLists, actives))
          .to.emit(asset, "ExternalControlListsUpdated")
    ...
    describe("View/Getter Functions", () => {
      ...
      it("GIVEN external control lists WHEN getExternalControlListsCount is called THEN it returns the current count", async () => {
        ...
        expect(await asset.getExternalControlListsCount()).to.equal(2);
      });
    ...
    describe("Pause Tests", () => {
      it("GIVEN a paused token WHEN addExternalControlList THEN it reverts with IsPaused", async () => {
        ...
        const newControlList = externalWhitelistMock2.target as string;
        await expect(asset.connect(signer_A).addExternalControlList(newControlList)).to.be.revertedWithCustomError(
          asset,
          "IsPaused",
        );
      });
    ...
    describe("Initialize Tests", () => {
      it("GIVEN an already initialized contract WHEN initializeExternalControlLists is called again THEN it reverts with FacetAlreadyRegistered", async () => {
      ...
      it("GIVEN a caller without DEFAULT_ADMIN_ROLE WHEN initializeExternalControlLists is called THEN it reverts with AccountHasNoRole", async () => {
        await expect(
          asset.connect(signer_D).initializeExternalControlLists([initMock1.target as string]),
    ...
    describe("Access Control Tests", () => {
      it("GIVEN an account without _CONTROL_LIST_MANAGER_ROLE WHEN adding an external control list THEN it reverts", async () => {
        const newControlList = externalWhitelistMock2.target as string;
        await expect(asset.connect(signer_B).addExternalControlList(newControlList)).to.be.revertedWithCustomError(
          asset,
          "AccountHasNoRole",
        );
      });
      ...
      it("GIVEN an account with _CONTROL_LIST_MANAGER_ROLE WHEN removing an external control list THEN it succeeds", async () => {
        expect(await asset.isExternalControlList(externalWhitelistMock1.target as string)).to.be.true;
        await expect(asset.connect(signer_A).removeExternalControlList(externalWhitelistMock1.target as string))
          .to.emit(asset, "RemovedFromExternalControlLists")
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN updateExternalControlLists THEN transaction fails with Deactivated", async () => {
        await expect(asset.connect(signer_A).updateExternalControlLists([], [])).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
      ...
      it("GIVEN a deactivated asset WHEN removeExternalControlList THEN transaction fails with Deactivated", async () => {
        await expect(asset.connect(signer_A).removeExternalControlList(ADDRESS_ZERO)).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
