...
import { ATS_ROLES, DEFAULT_PARTITION, EMPTY_HEX_BYTES, EMPTY_STRING, ZERO, RESOLVER_KEYS } from "@lib";
...
export function burnByPartitionTests(getCtx: () => AssetMockCtx): void {
  describe("BurnByPartitionFacet Tests", () => {
    ...
    describe("Multi partition mode", () => {
      ...
      it("GIVEN multi-partition mode WHEN redeemByPartition from non-default partition THEN succeeds and labafs updated correctly", async () => {
        ...
        const totalBalanceByPartition_1_after = await asset.getTotalBalanceForByPartition(
          CUSTOM_PARTITION,
          tokenHolder,
        );
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN redeemByPartition THEN transaction fails with Deactivated", async () => {
        await expect(asset.connect(signer_A).redeemByPartition(ethers.ZeroHash, 0, "0x")).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
    ...
    describe("initializeBurnByPartition", () => {
      ...
      it("GIVEN caller without DEFAULT_ADMIN_ROLE WHEN initializeBurnByPartition is called THEN AccountHasNoRole", async () => {
        await expect(asset.connect(unknownSigner).initializeBurnByPartition())
    ...
    describe("nonOperational", () => {
      ...
      it("GIVEN non-operational WHEN redeemByPartition is called THEN AssetNotOperational", async () => {
        await expect(asset.redeemByPartition(ethers.ZeroHash, 0n, "0x")).to.be.revertedWithCustomError(
          asset,
          "AssetNotOperational",
        );
      });
