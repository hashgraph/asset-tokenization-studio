...
import { deployEquityFromFactory, ATS_ROLES, CONFIG_IDS } from "@lib/domain";
...
describe("Equity Token Deployment", () => {
  ...
  describe("factory call", () => {
    ...
    it("should not hand-set gas options (provider estimates)", async () => {
      const diamondAddress = TEST_ADDRESSES.VALID_3;
      const mockFactory = createMockFactory(TEST_FACTORY_EVENTS.EQUITY_DEPLOYED, diamondAddress);
      const params = createDeployEquityParams(mockFactory);
      const regulationData = createMockRegulationData();
      ...
      // Third argument is overrides: empty outside coverage runs, so the
      // provider estimates gas
      expect(callArgs[2]).to.not.have.property("gasPrice");
      if (!process.env.COVERAGE) {
        expect(callArgs[2]).to.not.have.property("gasLimit");
      }
    });
  });
