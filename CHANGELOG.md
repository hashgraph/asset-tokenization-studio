...
- Hash codegen retired (ADR 0008): resolver-key and role hashes are pasted by hand (`npx hardhat ats:hash <kind> <Arg>` computes them) into both the `.sol` constant and its TypeScript mirror; a comparator test (`hashConsistency.test.ts`) — not a rewriter — is the CI gate.

### Major Changes

- Retire the hash codegen (ADR 0008): the Solidity-rewriting pass and its stability guard under `lib/codegen/` are deleted, along with the npm scripts and Hardhat task that drove them (and the pre-compile hook) — `npm run compile` is now a plain `hardhat compile`, nothing else. Resolver keys and roles are pasted by hand into both the `.sol` constant and its TypeScript mirror (`lib/domain/facetKeys.ts#RESOLVER_KEYS_BY_FACET`, `lib/domain/roles.ts#ROLES` — the latter renamed from the old auto-generated role file, now hand-maintained with no `@generated` header); the new `npx hardhat ats:hash <kind> <Arg>` task computes the hex and the canonical Solidity identifier to paste. A new comparator test (`test/scripts/unit/domain/hashConsistency.test.ts`) replaces the old rewriter's verification mode: it fails loudly if a TS literal, its `.sol` counterpart, or the formula disagree, without writing anything. `hashGen.ts` (the formula itself) is unchanged and still the single source of truth.
- Remove dead `lib/` exports left over from the ADR 0007/0008 cleanup — none had any consumer left in this repo (Hardhat tasks, Ignition, or tests). BREAKING for anyone importing them directly: `lib/infrastructure/signer.ts` (`createNetworkSigner`, `NetworkSignerResult`) and `lib/infrastructure/utils/decodeCustomError.ts` (`decodeCustomError`) are deleted outright — use `hre.ethers.getSigners()` or `new ethers.Wallet(...)` for signers; `createSigner` / `createSignerFromEnv` / `SignerOptions` (duplicates of the deleted `signer.ts`) are removed from `lib/infrastructure/types/core.ts`; `ATS_CONTRACTS` (`lib/domain/constants.ts`), `TOTAL_ROLES` (`lib/domain/roles.ts`), `toTypeChainLibraryAddresses` (`lib/domain/orchestratorLibraries.ts`), `DepositTokenDataParams` (`lib/domain/factory/types.ts`), `configurationExists` (`lib/infrastructure/operations/blrConfigurations.ts`), `verifyContract` / `verifyContractInterface` (`lib/infrastructure/utils/verification.ts` — `verifyContractCode` is unaffected), and `MAX_UINT8` / `INFRASTRUCTURE_CONTRACT_NAMES` / `PROXY_CONTRACTS` / `ENV_VAR_PATTERNS` / `DEPLOYMENT_OUTPUT_DIR` / `TIME_PERIODS_MS` (`lib/infrastructure/constants.ts`) and `FacetConfiguration` / `CreateBlrConfigurationResult` (`lib/infrastructure/types/blr.ts`) are all removed with no replacement. `createInitializeMockConfiguration` (`lib/domain/initializeMock/createConfiguration.ts`) is also removed; the still-used `INITIALIZE_MOCK_FACETS` constant moves to `lib/domain/initializeMock/mockFacetsRegistry.ts` and the source file is deleted.
- 1926e4f: Delete the generated contract-registry pipeline (ADR 0007): `atsRegistry.generated.ts` (~17.3k gitignored lines) and `lib/tools/registry-generator/` (scanner, Solidity parser, generator, cache) are gone; `atsRegistry.ts` becomes a thin, eager facade over the explicit `lib/domain/facetKeys.ts` facet -> resolver-key map, computed straight from the hash formula in `lib/codegen/hashGen.ts` (no lazy `require`s, no `Proxy`, no BBND-1766 machinery). BREAKING: `FACET_REGISTRY`, `INFRASTRUCTURE_CONTRACTS`, `STORAGE_WRAPPER_REGISTRY`, `FACETS`, `FacetKey`, `getContractDefinition`, `getAllContracts`, `hasContract`, `getStorageWrapperDefinition`, `getAllStorageWrappers`, `hasStorageWrapper`, `getFacetRegistryCount`, `getStorageWrapperRegistryCount`, `createRegistryHelpers`, and the entire `registry-generator` public surface (`generateRegistryPipeline`, `DEFAULT_CONFIG`, `CacheManager`, `LogLevel`, `configureLogger`, and the `RegistryConfig` / `RegistryResult` / `CacheEntry` / `RegistryCache` types) are removed with no replacement; the `MethodDefinition`, `EventDefinition`, `ErrorDefinition`, `ContractDefinition`, `StorageWrapperDefinition` types are gone and `FacetDefinition` slims to `{ name, resolverKey? }` (drops `description`, `roleCount`, `inheritance`, `methods`, `events`, `errors`, `factory`) — use TypeChain (`XFacet__factory.createInterface()`) for ABIs, selectors, and events instead. Migration: `getFacetDefinition(name).resolverKey.value` -> `getResolverKey(name)`; `FACET_REGISTRY[name]` / `FACETS.xxx` -> `getResolverKey(name)`, or `ALL_FACETS` for the full facet-name list. `RESOLVER_KEYS`, `atsRegistry`, `RegistryProvider`, and `combineRegistries` are unchanged.
...
- Remove the PoC-era Ignition companions: `ignition/verifyPoc.ts` (only valid against a virgin genesis — any later upgrade breaks its expected-version arithmetic; use `ats:blr:info` instead) and `ignition/exportDeployment.ts` (wrote the legacy `deployments/` JSON nobody consumes), along with the `ats:ignition:verify` / `ats:ignition:export` tasks, their npm scripts, and the now-dead `lib` helpers (`deploymentFiles.ts`, `paths.ts`, `DeploymentOutputType` / `SaveDeploymentOptions` / `SaveResult`).
- Replace the `AtsExistingBlrSystem` Ignition module with the `ats:blr:deploy-system` task. The module needed a build-time `EXISTING_BLR_VERSION_OFFSET` env var (a human-verified assumption about the target BLR's registration history, wrong for BLRs with per-facet upgrades); the task reads every key's current version from the chain instead, per ADR 0005. `ignition/params/local.json` is deleted with it. `PRODUCTION_DEPLOY_FACETS` (the deployable production facet list, previously internal to `ignition/lib/shared.ts`) moves to `lib/domain/facetKeys.ts`, and `deployResolverProxy` gains a `replacementEnabled` option (default `false`, unchanged behaviour).
...
- Memoize hardhat-ignition's build-info resolution (in `hardhat.config.ts`): the stock resolver re-reads and re-parses the single ~90 MB build-info JSON once per contract future, ~115 times per full-system deploy, which multiplied every Ignition deploy and every test-fixture deploy by ~12x (a fixture-heavy test file went from 12 s pre-Ignition to 2 m 25 s; back to ~24 s with the cache). Also lower Ignition's `blockPollingInterval` to 50 ms on instant-mining networks (kept at the 1 s default against Hedera relays).
