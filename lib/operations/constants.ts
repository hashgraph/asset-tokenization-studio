...
/**
 * Constants for ATS deployment system: network configuration, deployment
 * defaults, and other system-wide values.
 */
...
/**
 * Number of facets registered per `registerBusinessLogics` transaction.
 *
 * Generic batch size for facet registration (no ATS-specific meaning). Kept
 * smaller than {@link DEFAULT_BATCH_SIZE} because a registration payload is
 * heavier per entry than a configuration batch. Raise for nodes with a higher
 * gas limit.
 */
