...
/**
 * Role hash registry, mirroring every `bytes32 constant ROLE_*` in
 * `contracts/constants/roles.sol` (plus the hand-written OpenZeppelin
 * `DEFAULT_ADMIN_ROLE` sentinel) one-for-one. Maintained by hand — no codegen
 * writes this file. Each value is pasted by hand from the same formula the
 * `.sol` constant uses (`HASHES.role(<Arg>)` in `lib/codegen/hashGen.ts`,
 * matching the `/// @custom:hash role <Arg>` annotation above the constant) —
 * compute it with `npx hardhat ats:hash role <Arg>`.
 *
 * `test/scripts/unit/domain/hashConsistency.test.ts` fails with an
 * actionable message if this file and `roles.sol` drift apart, or if either
 * one drifts from the formula.
 *
 * Import from `@lib/domain` instead of this file directly.
 */
...
export const ROLES = {
  ...
  ROLE_CREATE_CONFIGURATION: "0x185bc02f8b16b873d7c8b9a6cb21f91bb77dcec0ee1f4af6cc568f084b1da9f8",
