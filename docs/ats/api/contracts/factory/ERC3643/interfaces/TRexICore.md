...
```solidity
function getERC20Metadata() external view returns (struct TRexICore.ERC20Metadata)
```
...
```solidity
function initializeCore(TRexICore.ERC20Metadata metadata) external nonpayable
```
...
```solidity
event CoreInitialized(TRexICore.ERC20Metadata metadata)
```
