...
Exposes Diamond management, loupe, and ERC-165 selectors for proxy registration.

_Provides static resolver metadata for the core Diamond facet under `RESOLVER_KEY_DIAMOND`. The returned selectors include Diamond cut, resolver configuration, Diamond loupe pagination, facet lookup, and interface support functions inherited through `DiamondBase`._
...
_The selector count must remain aligned with the number of assigned selectors._
...
_Registers the Diamond, Diamond cut, Diamond loupe, and ERC-165 interface IDs._
