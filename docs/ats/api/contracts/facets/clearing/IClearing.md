...
Interface for the clearing module global state and account-level read queries.

_Exposes the one-shot initializer (`initializeClearing`), the lifecycle toggles (`activateClearing` / `deactivateClearing`), the activation status query (`isClearingActivated`), and the account-scoped read queries (`getClearedAmountFor`, `getClearingThirdParty`). Partition-scoped operations and reads remain on `IClearingByPartition`._
