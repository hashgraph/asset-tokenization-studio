...
Interface for partition-scoped clearing operations: approve, cancel, reclaim, clearing redeems and transfers, and read queries scoped to a single partition.

_Extends IClearingTypes. Implementations must enforce single-partition mode, protected-partition access control, and standard clearing lifecycle guards._
