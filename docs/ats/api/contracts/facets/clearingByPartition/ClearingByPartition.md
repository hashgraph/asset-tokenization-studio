...
Provides partition-scoped clearing lifecycle, creation, and query operations.

_Implements `IClearingByPartition` through shared clearing orchestrators and facet initialisation storage. Mutating entry points depend on operational, activation, pause, partition, identity, recovery, role, and clearing-state modifiers._
