...
Diamond facet exposing principal queries via `IPrincipal`, registered under `RESOLVER_KEY_PRINCIPAL`.

_Consolidates `getPrincipalFor` into a dedicated facet registered under `RESOLVER_KEY_PRINCIPAL`. Exposes 1 selector: `getPrincipalFor`._
