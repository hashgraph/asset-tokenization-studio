...
Diamond facet exposing snapshot-aware frozen balance queries via `IFreezeAtSnapshot`, registered under `RESOLVER_KEY_FREEZE_AT_SNAPSHOT`.

_Consolidates `frozenBalanceOfAtSnapshot` previously hosted in `SnapshotsFacet`. Exposes 1 selector: `frozenBalanceOfAtSnapshot`._
