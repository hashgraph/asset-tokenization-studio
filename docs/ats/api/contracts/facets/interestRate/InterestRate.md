...
Abstract contract implementing the admin-controlled coupon rate type selector.

_Implements `IInterestRate`. State is stored via `InterestRateStorageWrapper` at `STORAGE_LOCATION_INTEREST_RATE_TYPE`. Intended to be inherited exclusively by `InterestRateFacet`._
