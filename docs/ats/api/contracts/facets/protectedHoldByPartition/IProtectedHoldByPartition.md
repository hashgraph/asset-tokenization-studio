...
Interface for the protected variant of partition-scoped hold creation, gated by a per-partition role and an off-chain signature provided by the token holder.

_Inherits the shared `IHoldTypes` module so the `ProtectedHold` struct and the `ProtectedHeldByPartition` event are referenced from a single source of truth and never redeclared. This interface is aggregated into the `IAsset` umbrella; the facet itself never inherits the umbrella._
