...
Diamond facet that registers the `protectedCreateHoldByPartition` selector under `RESOLVER_KEY_PROTECTED_HOLD_BY_PARTITION` on the Diamond proxy.

_Composed via `ProtectedHoldByPartition` for behaviour and `IStaticFunctionSelectors` for selector advertisement. Exposes a single external selector._
