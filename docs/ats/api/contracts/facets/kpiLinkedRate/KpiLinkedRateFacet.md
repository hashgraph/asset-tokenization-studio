...
Diamond facet that exposes the KPI-linked interest rate capability (`IKpiLinkedRate`) on a token.

_Implements `IStaticFunctionSelectors` so the BusinessLogicResolver can register the facet&#39;s selectors against the deterministic resolver key declared at file scope in the facet&#39;s interface (`IKpiLinkedRate`)._
