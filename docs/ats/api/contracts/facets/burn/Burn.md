...
Abstract implementation of the ERC-1594 redemption and ERC-3643 burn surfaces. Exposes three entry points: `burn` (controller/agent-initiated), `redeem` (self-redemption by the caller) and `redeemFrom` (operator-initiated on behalf of a token holder). All operations are restricted to single-partition mode.
