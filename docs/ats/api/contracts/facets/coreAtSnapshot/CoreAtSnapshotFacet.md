...
Diamond facet that exposes core token properties resolved against a snapshot identifier, registered under `RESOLVER_KEY_CORE_AT_SNAPSHOT`.

_Inherits read logic from `CoreAtSnapshot` and satisfies `IStaticFunctionSelectors` for Diamond proxy selector registration. Exposes one selector: `decimalsAtSnapshot`._
