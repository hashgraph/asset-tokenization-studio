...
Diamond facet that exposes controller-initiated forced transfer and redemption operations on a specific partition, registered under `RESOLVER_KEY_CONTROLLER_BY_PARTITION`.

_Inherits behaviour from `ControllerByPartition` and satisfies `IStaticFunctionSelectors` for Diamond proxy selector registration. Exposes two selectors: `controllerTransferByPartition`, `controllerRedeemByPartition`._
