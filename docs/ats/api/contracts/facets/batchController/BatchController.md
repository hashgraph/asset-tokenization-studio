...
_Shares the access-control/role semantics of `IERC1644.controllerTransfer`: the caller must hold either the controller or the agent role, the token must be controllable and operating in single-partition mode. Emits one `ControllerTransfer` event per transferred element so downstream indexers observe the same event stream as the single-shot path._
