# ProtectedClearingHoldByPartition

_Asset Tokenization Studio Team_

> ProtectedClearingHoldByPartition

Abstract facet implementation for the protected variant of partition-scoped clearing hold creation, extracted from `ClearingHoldCreation` as part of the MAF (Modular Asset Factory) decomposition.

_Forwards write logic to `ClearingProtectedOps.protectedClearingCreateHoldByPartition`. Authorisation is enforced by a partition-specific role obtained from `ProtectedPartitionsStorageWrapper`. Storage layout is unchanged; this contract only owns the selector exposure._
