...
Diamond facet that consolidates all 8 scheduled balance-adjustment selectors under a single `RESOLVER_KEY_SCHEDULED_BALANCE_ADJUSTMENT`.

_Inherits implementation from `ScheduledBalanceAdjustment` and satisfies the `IStaticFunctionSelectors` contract required by the Diamond proxy for selector registration. Exposes: `setScheduledBalanceAdjustment`, `cancelScheduledBalanceAdjustment`, `getScheduledBalanceAdjustment`, `getBalanceAdjustmentCount`, `getPendingBalanceAdjustmentCount`, `getScheduledBalanceAdjustments`._
