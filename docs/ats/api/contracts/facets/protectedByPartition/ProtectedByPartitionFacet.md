...
Diamond facet that registers the protected partition-scoped transfer and redemption selectors under `RESOLVER_KEY_PROTECTED_BY_PARTITION` on the Diamond proxy.

_Composed via `ProtectedByPartition` for behaviour and `IStaticFunctionSelectors` for selector advertisement. Exposes two external selectors: - `protectedTransferFromByPartition` - `protectedRedeemFromByPartition`_
