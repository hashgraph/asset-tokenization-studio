...
Diamond facet that exposes snapshotted partition-scoped balance and total-supply queries through the `IBalanceTrackerAtSnapshotByPartition` interface, registered under `RESOLVER_KEY_BALANCE_TRACKER_AT_SNAPSHOT_BY_PARTITION`.

_Inherits read logic from `BalanceTrackerAtSnapshotByPartition` and satisfies `IStaticFunctionSelectors` for Diamond proxy selector registration. Exposes two selectors: `balanceOfAtSnapshotByPartition`, `totalSupplyAtSnapshotByPartition`._
