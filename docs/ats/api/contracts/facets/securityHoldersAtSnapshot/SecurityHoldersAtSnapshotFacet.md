...
Diamond facet that exposes paginated token-holder queries at a given snapshot via `ISecurityHoldersAtSnapshot`, registered under `RESOLVER_KEY_SECURITY_HOLDERS_AT_SNAPSHOT`.

_Exposes two selectors: `getTokenHoldersAtSnapshot` and `getTotalTokenHoldersAtSnapshot`. Inherits read logic from `SecurityHoldersAtSnapshot` and satisfies `IStaticFunctionSelectors` for Diamond proxy selector registration._
