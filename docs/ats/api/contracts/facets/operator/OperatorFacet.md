...
Diamond facet that exposes operator management operations as selectable proxy functions.

_Inherits `Operator` for the business logic and implements `IStaticFunctionSelectors` for the Diamond resolver pattern. The resolver key `RESOLVER_KEY_OPERATOR` identifies this facet within the diamond proxy._
