...
Abstract implementation of `IClearingAtSnapshot` providing the snapshotted aggregate cleared-balance query indexed by a snapshot identifier.

_Delegates storage reads to `SnapshotsStorageWrapper`. Intended to be inherited by `ClearingAtSnapshotFacet`._
