...
Abstract implementation of `IAdjustBalances` providing immediate balance adjustment corporate actions for tokenised assets.

_Inherits access-control guards from `Modifiers`. Immediate adjustments are applied via `AdjustBalancesStorageWrapper`. Scheduled adjustments are handled by `IScheduledBalanceAdjustment`. Intended to be inherited by `AdjustBalancesFacet`._
