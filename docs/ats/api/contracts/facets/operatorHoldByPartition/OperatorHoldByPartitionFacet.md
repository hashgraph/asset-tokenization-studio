...
Diamond facet that exposes operator-initiated hold creation on a partition via `IOperatorHoldByPartition`, registered under `RESOLVER_KEY_OPERATOR_HOLD_BY_PARTITION`.

_Exposes one selector: `operatorCreateHoldByPartition`. Must be registered in the BusinessLogicResolver before use._
