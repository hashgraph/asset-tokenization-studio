...
Diamond facet that registers the `protectedClearingRedeemByPartition` and `protectedClearingTransferByPartition` selectors under `RESOLVER_KEY_PROTECTED_CLEARING_BY_PARTITION` on the Diamond proxy.

_Composed via `ProtectedClearingByPartition` for behaviour and `IStaticFunctionSelectors` for selector advertisement. Exposes two external selectors._
