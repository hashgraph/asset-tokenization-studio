...
Interface for the protected variant of partition-scoped clearing operations (redeem and transfer), gated by a per-partition role and an off-chain signature provided by the token holder.

_The protected-clearing events (`ProtectedClearedRedeemByPartition`, `ProtectedClearedTransferByPartition`) are declared on this writer interface and emitted inline from `ProtectedClearingByPartition.protectedClearing{Redeem,Transfer}ByPartition` after the `ClearingProtectedOps` library call returns successfully. `is IClearingTypes` is retained for the `ProtectedClearingOperation` struct used in method signatures._
