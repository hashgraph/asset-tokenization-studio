...
Migrate all facet initialization to centralised `InitializerStorageWrapper` pattern across
...
**New `initializeXxx` added** (no prior initialization existed): AdjustBalances, Allowance,
...
CouponListing) gain `Modifiers` inheritance to support the initializer guards.
