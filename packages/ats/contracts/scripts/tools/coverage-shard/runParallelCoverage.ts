...
// place, so N shards in one checkout would corrupt each other), then merges the per-shard lcovs into
// the main checkout's coverage/lcov.info. node_modules is symlinked, not reinstalled. See ./README.md
// for the full rationale (worktree isolation, path normalisation, the merge).
//
...
import { mkdtempSync, rmSync, mkdirSync, symlinkSync, existsSync, readFileSync, writeFileSync } from "fs";
...
/** Create a detached worktree at HEAD (avoids the "branch already checked out" conflict; mirrors CI,
 *  which runs committed code — uncommitted changes are NOT included). */
...
async function runShard(index: number, baseDir: string): Promise<ShardResult> {
...
async function runAll(baseDir: string): Promise<ShardResult[]> {
  const results: ShardResult[] = [];
  let next = 0;
  async function worker(): Promise<void> {
    while (next < shardTotal) {
      results.push(await runShard(next++, baseDir));
    }
  }
...
async function main(): Promise<void> {
  ...
  if (dirty) {
    process.stdout.write(
      "runParallelCoverage: working tree is dirty — shards run against HEAD (committed code), so " +
        "uncommitted changes are NOT measured. Commit them first if you need them covered.\n",
    );
  }
  ...
  try {
    const results = await runAll(baseDir);
