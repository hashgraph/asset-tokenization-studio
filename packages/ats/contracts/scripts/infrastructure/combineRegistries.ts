...
// Implementation
// eslint-disable-next-line no-redeclare
export function combineRegistries(...args: Array<RegistryProvider | CombineRegistriesOptions>): RegistryProvider {
  // Parse arguments (last arg might be options)
  const lastArg = args[args.length - 1];
  const hasOptions =
    lastArg && typeof lastArg === "object" && "onConflict" in lastArg && !("getFacetDefinition" in lastArg);

  const registries = hasOptions ? (args.slice(0, -1) as RegistryProvider[]) : (args as RegistryProvider[]);
  const options: CombineRegistriesOptions = hasOptions ? (lastArg as CombineRegistriesOptions) : {};
    ...
    for (const facet of facets) {
      ...
      if (existing) {
        ...
        switch (onConflict) {
          ...
          case "warn":
            warn(`Registry conflict: Facet '${facet.name}' appears in registry ${i}. Using last definition.`);
 ...
 * ```
 */
export function getRegistryConflicts(...registries: RegistryProvider[]): string[] {
