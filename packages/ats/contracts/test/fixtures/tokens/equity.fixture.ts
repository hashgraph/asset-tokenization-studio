...
export function getEquityDetails(
    params?: DeepPartial<EquityDetailsDataParams>
) {
    return {
        votingRight: params?.votingRight ?? DEFAULT_EQUITY_PARAMS.votingRight,
        informationRight:
            params?.informationRight ?? DEFAULT_EQUITY_PARAMS.informationRight,
        liquidationRight:
            params?.liquidationRight ?? DEFAULT_EQUITY_PARAMS.liquidationRight,
        subscriptionRight:
            params?.subscriptionRight ??
            DEFAULT_EQUITY_PARAMS.subscriptionRight,
        conversionRight:
            params?.conversionRight ?? DEFAULT_EQUITY_PARAMS.conversionRight,
        redemptionRight:
            params?.redemptionRight ?? DEFAULT_EQUITY_PARAMS.redemptionRight,
        putRight: params?.putRight ?? DEFAULT_EQUITY_PARAMS.putRight,
        dividendRight:
            params?.dividendRight ?? DEFAULT_EQUITY_PARAMS.dividendRight,
        currency: params?.currency ?? DEFAULT_EQUITY_PARAMS.currency,
        nominalValue:
            params?.nominalValue ?? DEFAULT_EQUITY_PARAMS.nominalValue,
    }
}
/**
