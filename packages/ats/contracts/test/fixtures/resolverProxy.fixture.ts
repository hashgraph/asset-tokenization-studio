...
import { DiamondCutFacet__factory, AccessControlFacet__factory, BusinessLogicResolver__factory } from "@contract-types";
import type { BusinessLogicResolver, DiamondCutFacet, AccessControlFacet } from "@contract-types";
...
export async function deployResolverProxyFixture(): Promise<ResolverProxyFixtureResult> {
  ...
  const facetNames = ["DiamondCutFacet", "DiamondFacet", "AccessControlFacet"];
  ...
  const ResolverProxyFactory = await ethers.getContractFactory("ResolverProxy", deployer);
  ...
  const diamondCutFacet = DiamondCutFacet__factory.connect(resolverProxy.address, deployer);
