...
describe('Factory Tests', () => {
    ...
    describe('Equity tests', () => {
        ...
        it('GIVEN the proper information WHEN deploying a new resolverProxy THEN transaction succeeds', async () => {
            ...
            expect(metadata.info.name).to.be.equal(
                equityData.security.erc20MetadataInfo.name
            )
            expect(metadata.info.symbol).to.be.equal(
                equityData.security.erc20MetadataInfo.symbol
            )
            expect(metadata.info.decimals).to.be.equal(
                equityData.security.erc20MetadataInfo.decimals
            )
            expect(metadata.info.isin).to.be.equal(
                equityData.security.erc20MetadataInfo.isin
            )
            ...
            expect(equityMetadata.votingRight).to.equal(
                equityData.equityDetails.votingRight
            )
            expect(equityMetadata.informationRight).to.equal(
                equityData.equityDetails.informationRight
            )
            expect(equityMetadata.liquidationRight).to.equal(
                equityData.equityDetails.liquidationRight
            )
            expect(equityMetadata.subscriptionRight).to.equal(
                equityData.equityDetails.subscriptionRight
            )
            expect(equityMetadata.conversionRight).to.equal(
                equityData.equityDetails.conversionRight
            )
            expect(equityMetadata.redemptionRight).to.equal(
                equityData.equityDetails.redemptionRight
            )
            expect(equityMetadata.putRight).to.equal(
                equityData.equityDetails.putRight
            )
            expect(equityMetadata.dividendRight).to.equal(
                equityData.equityDetails.dividendRight
            )
            expect(equityMetadata.currency).to.equal(
                equityData.equityDetails.currency
            )
            expect(equityMetadata.nominalValue).to.equal(
                equityData.equityDetails.nominalValue
            )
    ...
    describe('Bond tests', () => {
        ...
        it('GIVEN the proper information WHEN deploying a new bond THEN transaction succeeds', async () => {
            ...
            expect(metadata.info.isin).to.be.equal(
                bondData.security.erc20MetadataInfo.isin
            )
