...
describe('ERC20 Tests', () => {
  ...
  describe('Multi partition', () => {
    before(async () => {
      ...
      console.log = () => {};
      [signer_A, signer_B, signer_C, signer_E] = await ethers.getSigners();
    ...
    it('GIVEN a paused ERC20 WHEN running any state changing method THEN transaction fails with TokenIsPaused', async () => {
      ...
      await expect(erc20Facet.approve(account_E, amount)).to.be.rejectedWith(
        'TokenIsPaused',
      );

      await expect(erc20Facet.transfer(account_E, amount)).to.be.rejectedWith(
        'TokenIsPaused',
      );
  ...
  describe('Single partition', () => {
    ...
    describe('Approval', () => {
      it(
        ...
        async () => {
          await expect(
            erc20SignerC.approve(ethers.constants.AddressZero, amount / 2),
      ...
      it(
        ...
        async () => {
          await expect(erc20SignerE.decreaseAllowance(account_B, amount / 2))
      ...
      it(
        'GIVEN a account with balance ' +
          ...
          'THEN emits Approval event and allowance is updated',
        async () => {
          expect(await erc20SignerC.increaseAllowance(account_E, amount / 2))
            .to.emit(erc20SignerC, 'Approval')
            .withArgs(account_C, account_E, amount / 2);
          expect(
            await erc20SignerC.allowance(account_C, account_E),
      ...
      it(
        ...
        async () => {
          ...
          expect(await erc20SignerC.decreaseAllowance(account_E, amount / 2))
            .to.emit(erc20SignerC, 'Approval')
            .withArgs(account_C, account_E, amount / 2);
          expect(
            await erc20SignerC.allowance(account_C, account_E),
    ...
    describe('transferFrom', () => {
      ...
      it(
        ...
        async () => {
          expect(
            await erc20SignerE.transferFrom(account_C, account_E, amount / 2),
          )
            .to.emit(erc20SignerC, 'Transfer')
            .withArgs(account_C, account_E, amount);
          expect(await erc1410Facet.balanceOf(account_C)).to.be.equal(
