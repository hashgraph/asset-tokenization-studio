  ...
  describe('single partition', () => {
    ...
    describe('ForcedTransfer', () => {
      ...
      it('GIVEN a paused token WHEN attempting to forcedTransfer TokenIsPaused error', async () => {
        ...
        await expect(
          erc3643Facet.forcedTransfer(account_A, account_B, AMOUNT - 1),
    ...
    describe('Freeze', () => {
      ...
      it('GIVEN a valid address WHEN attempting to freezePartialTokens THEN transactions succeed', async () => {
        ...
        expect(await freezeFacet.getFrozenTokens(account_E)).to.be.equal(
          amount,
        );
      ...
      it('GIVEN a valid address WHEN attempting to unfreezePartialTokens THEN transactions succeed', async () => {
        ...
        await expect(freezeFacet.unfreezePartialTokens(account_E, amount))
          .to.emit(freezeFacet, 'TokensUnfrozen')
          .withArgs(account_E, amount, DEFAULT_PARTITION);
        expect(await freezeFacet.getFrozenTokens(account_E)).to.be.equal(0);
      ...
      it('GIVEN a freeze amount greater than balance WHEN attempting to unfreezePartialTokens THEN transactions revert with InsufficientFrozenBalance error', async () => {
        ...
        await freezeFacet.freezePartialTokens(account_E, amount);
        await expect(freezeFacet.unfreezePartialTokens(account_E, amount + 1))
    ...
    describe('Identity', () => {
      ...
      it('GIVEN non verified account with balance WHEN transfer THEN reverts with AddressNotVerified', async () => {
        ...
        await erc1410Facet.connect(signer_E).authorizeOperator(account_D);
        ...
        await expect(
          erc3643Facet.connect(signer_E).batchTransfer([account_D], [AMOUNT]),
        ).to.be.revertedWithCustomError(erc3643Facet, 'AddressNotVerified');
      });
      ...
      it('GIVEN non verified account WHEN redeem THEN reverts with AddressNotVerified', async () => {
        ...
        await expect(
          erc1410Facet
            .connect(signer_E)
            .redeemByPartition(DEFAULT_PARTITION, AMOUNT, EMPTY_HEX_BYTES),
        ).to.be.revertedWithCustomError(erc3643Facet, 'AddressNotVerified');
        await expect(
          erc1594Facet.connect(signer_E).redeem(AMOUNT, EMPTY_HEX_BYTES),
        ).to.be.revertedWithCustomError(erc3643Facet, 'AddressNotVerified');
        await expect(
          erc1594Facet
            .connect(signer_D)
    ...
    describe('ERC3643 canTransfer Compliance Integration', () => {
      ...
      it('GIVEN ComplianceMock.canTransfer returns true THEN transfers succeed', async () => {
        ...
        await expect(erc20FacetE.transfer(account_D, AMOUNT / 2)).to.not.be
          .reverted;

        expect(await erc1410SnapshotFacet.balanceOf(account_E)).to.equal(
          AMOUNT / 2,
        );
        expect(await erc1410SnapshotFacet.balanceOf(account_D)).to.equal(
          AMOUNT / 2,
        );
      });
      ...
      it('GIVEN zero address compliance THEN transfers succeed without compliance checks', async () => {
        ...
        await accessControlNoCompliance.grantRole(SSI_MANAGER_ROLE, account_A);
        ...
        await expect(erc20NoCompliance.transfer(account_D, AMOUNT / 2)).to.not
          .be.reverted;
      });
    });
    ...
    describe('Compliance', () => {
      it('GIVEN an initialized token WHEN updating the compliance THEN setCompliance emits ComplianceAdded with updated compliance', async () => {
        ...
        expect(await erc3643Facet.setCompliance(newComplianceMockAddress))
        ...
        expect(retrieved_newCompliance).to.equal(newComplianceMockAddress);
      });
      ...
      it('GIVEN a successful transfer THEN transferred is called in compliance contract', async () => {
        ...
        await erc20Facet.connect(signer_D).approve(account_E, MAX_UINT256);
        await erc20Facet.connect(signer_E).approve(account_D, MAX_UINT256);
        await erc1410Facet.connect(signer_E).authorizeOperator(account_D);
        await erc1410Facet.connect(signer_D).authorizeOperator(account_E);
        // Issue
        await erc1410Facet.issueByPartition({
          partition: DEFAULT_PARTITION,
          tokenHolder: account_E,
        ...
        await erc1594Facet
          .connect(signer_E)
          .transferFromWithData(account_D, account_E, AMOUNT, EMPTY_HEX_BYTES);
        transfersCounter++;
        await erc3643Facet
          .connect(signer_E)
      ...
      it('GIVEN a failed canTransfer call THEN transaction reverts with custom error', async () => {
        ...
        let caught;
        try {
          await erc20Facet.connect(signer_E).approve(account_D, AMOUNT);
        } catch (err: any) {
          caught = err;
        }
        const returnedSelector = (caught.data as string).slice(0, 10);
      ...
      it('GIVEN ComplianceMock::canTransfer returns false THEN operations fail with ComplianceNotAllowed', async () => {
        ...
        await expect(
          erc1410Facet
            .connect(signer_E)
            .authorizeOperatorByPartition(DEFAULT_PARTITION, account_D),
        ).to.be.revertedWithCustomError(erc3643Facet, 'ComplianceNotAllowed');
        await expect(
    ...
    describe('Batch Operations', () => {
      describe('batchMint', () => {
        it('GIVEN an account with issuer role WHEN batchMint THEN transaction succeeds and balances are updated', async () => {
          ...
          await expect(erc3643Issuer.batchMint(toList, amounts)).to.not.be
            .reverted;
      ...
      describe('batchTransfer', () => {
        ...
        it('GIVEN a valid sender WHEN batchTransfer THEN transaction succeeds and balances are updated', async () => {
          ...
          erc3643Transferor = erc3643Facet.connect(signer_E);
          await expect(erc3643Transferor.batchTransfer(toList, amounts)).to.not
            .be.reverted;
      ...
      describe('batchForcedTransfer', () => {
        ...
        it('GIVEN controller role WHEN batchForcedTransfer THEN transaction succeeds', async () => {
          ...
          erc3643Facet = erc3643Facet.connect(signer_A);
          await expect(
            erc3643Facet.batchForcedTransfer(fromList, toList, amounts),
          ).to.not.be.reverted;
      ...
      describe('batchBurn', () => {
        ...
        it('GIVEN approved operator WHEN batchBurn THEN transaction succeeds', async () => {
          ...
          erc3643Facet = erc3643Facet.connect(signer_A);
          await expect(erc3643Facet.batchBurn(userAddresses, amounts)).to.not.be
            .reverted;
      ...
      describe('batchSetAddressFrozen', () => {
        ...
        it('GIVEN a FREEZE_MANAGER WHEN batchSetAddressFrozen with true THEN transfers from those addresses fail', async () => {
          ...
          erc3643Facet = erc3643Facet.connect(signer_A);
          await expect(
            freezeFacet.batchSetAddressFrozen(userAddresses, freezeFlags),
          ).to.not.be.reverted;
        ...
        it('GIVEN frozen addresses WHEN batchSetAddressFrozen with false THEN transfers from those addresses succeed', async () => {
          ...
          await expect(
            freezeFacet.batchSetAddressFrozen(userAddresses, [false, false]),
          ).to.not.be.reverted;

          const erc20FacetD = erc20Facet.connect(signer_D);
          await expect(erc20FacetD.transfer(account_A, transferAmount)).to.not
            .be.reverted;

          const erc20FacetE = erc20Facet.connect(signer_E);
          await expect(erc20FacetE.transfer(account_A, transferAmount)).to.not
            .be.reverted;
      ...
      describe('batchFreezePartialTokens', () => {
        ...
        it('GIVEN FREEZE_MANAGER_ROLE WHEN batchFreezePartialTokens THEN tokens are frozen successfully', async () => {
          ...
          await expect(
            freezeFacet.batchFreezePartialTokens(userAddresses, amounts),
          ).to.not.be.reverted;
      ...
      describe('batchUnfreezePartialTokens', () => {
        ...
        it('GIVEN frozen tokens WHEN batchUnfreezePartialTokens THEN tokens are unfrozen successfully', async () => {
          ...
          await expect(
            freezeFacet.batchUnfreezePartialTokens(userAddresses, amounts),
          ).to.not.be.reverted;
    ...
    describe('Agent', () => {
      it('GIVEN an initialized token WHEN adding agent THEN addAgent emits AgentAdded with agent address', async () => {
        ...
        const hasRole = await accessControlFacet.hasRole(AGENT_ROLE, account_B);
      ...
      it('GIVEN a user with the agent role WHEN performing actions using ERC-3643 methods succeeds', async () => {
        ...
        await expect(freezeFacet.unfreezePartialTokens(account_E, amount))
          .to.emit(freezeFacet, 'TokensUnfrozen')
          .withArgs(account_E, amount, DEFAULT_PARTITION);
        await expect(erc3643Facet.forcedTransfer(account_E, account_D, amount))
          .to.emit(erc1410Facet, 'TransferByPartition')
          .withArgs(
            DEFAULT_PARTITION,
            ADDRESS_ZERO,
    ...
    describe('AccessControl', () => {
      ...
      it('GIVEN an account without TREX_OWNER role WHEN setSymbol THEN transaction fails with AccountHasNoRole', async () => {
        ...
        // set symbol fails
        await expect(erc3643Facet.setSymbol(newSymbol)).to.be.rejectedWith(
          'AccountHasNoRole',
        );
      });
      it('GIVEN an account without TREX_OWNER role WHEN setOnchainID THEN transaction fails with AccountHasNoRole', async () => {
        ...
        // set onchainID fails
        await expect(erc3643Facet.setOnchainID(onchainId)).to.be.rejectedWith(
          'AccountHasNoRole',
        );
      });
      ...
      it('GIVEN an account without admin role WHEN addAgent THEN transaction fails with AccountHasNoRole', async () => {
        ...
        // add agent fails
        await expect(erc3643Facet.addAgent(account_A)).to.be.rejectedWith(
          'AccountHasNoRole',
        );
      });
    });
    ...
    describe('Adjust balances', () => {
      ...
      async function setPreBalanceAdjustment() {
        ...
        await accessControlFacet.grantRole(CONTROLLER_ROLE, account_A);
        await accessControlFacet.grantRole(CORPORATE_ACTION_ROLE, account_B);
        // Using account C (with role)
      ...
      it('GIVEN a freeze WHEN adjustBalances THEN frozen amount gets updated succeeds', async () => {
        ...
        const balance_Before_Partition_1 =
          await erc1410Facet.balanceOfByPartition(DEFAULT_PARTITION, account_E);
    ...
    describe('Recovery', () => {
      it('GIVEN lost wallet with pending locks, holds or clearings THEN recovery fails with CannotRecoverWallet', async () => {
        ...
        await expect(
          erc3643Facet.recoveryAddress(account_E, account_B, ADDRESS_ZERO),
      ...
      it('GIVEN lost wallet WHEN calling recovery using a previously recovered address THEN recovered status is set to false', async () => {
        await erc3643Facet.recoveryAddress(account_C, account_B, ADDRESS_ZERO);
        await erc3643Facet.recoveryAddress(account_B, account_C, ADDRESS_ZERO);
      ...
      it('GIVEN a recovered address THEN operations should fail', async () => {
        ...
        await accessControlFacet.grantRole(LOCKER_ROLE, account_A);
        await erc1410Facet.connect(signer_C).authorizeOperator(account_A);
        await erc1410Facet.connect(signer_C).authorizeOperator(account_B);
        await erc1410Facet.connect(signer_A).authorizeOperator(account_C);
        await erc1410Facet.connect(signer_A).authorizeOperator(account_A);
        const amount = 1000;
        // Recover
        await erc3643Facet.recoveryAddress(account_C, account_B, ADDRESS_ZERO);
        // Transfers
        ...
        await expect(
          erc20Facet.connect(signer_C).transfer(basicTransferInfo.to, amount),
        ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered');
        await expect(
          erc20Facet
            .connect(signer_C)
        ...
        await expect(
          erc3643Facet.connect(signer_C).batchTransfer([account_D], [amount]),
        ).to.revertedWithCustomError(erc3643Facet, 'WalletRecovered');
        // 2 - From
        ...
        await expect(
          erc1594Facet.transferWithData(account_C, amount, EMPTY_HEX_BYTES),
        ).to.revertedWithCustomError(erc3643Facet, 'WalletRecovered');
        await expect(
        ...
        // 1 - Operator
        await expect(
          erc20Facet.connect(signer_C).increaseAllowance(account_A, amount),
        ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered');
        await expect(
        ...
        await expect(
          erc1410Facet
            .connect(signer_C)
            .authorizeOperatorByPartition(DEFAULT_PARTITION, account_A),
        ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered');
        // 2 - To
        await expect(
        ...
        // 1 - Operator
        await expect(
          erc1594Facet.connect(signer_C).redeem(amount, EMPTY_HEX_BYTES),
        ).to.revertedWithCustomError(erc3643Facet, 'WalletRecovered');
        await protectedPartitionsFacet.protectPartitions();
        await expect(
        ...
        await expect(
          clearingFacet
            .connect(signer_C)
            .clearingRedeemFromByPartition(clearingOperationFrom, amount),
        ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered');
        await protectedPartitionsFacet.protectPartitions();
        await expect(
        ...
        await expect(
          holdFacet.executeHoldByPartition(holdIdentifier, account_C, amount),
        ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered');
        // Can transfer
        // 1 - Operator
        await erc1410Facet.issueByPartition({
  ...
  describe('multi partition', () => {
    ...
    describe('Batch operations', () => {
      ...
      it('GIVEN an single partition token WHEN batchForcedTransfer THEN transaction fails with NotAllowedInMultiPartitionMode', async () => {
        await expect(
          erc3643Facet.batchForcedTransfer([account_A], [account_A], [AMOUNT]),
        ).to.be.revertedWithCustomError(
          erc1410Facet,
          'NotAllowedInMultiPartitionMode',
        );
    ...
    describe('Freeze', () => {
      ...
      it('GIVEN an account with FREEZE_MANAGER_ROLE WHEN batchFreezePartialTokens THEN transaction fails with NotAllowedInMultiPartitionMode', async () => {
        await expect(
          freezeFacet.batchUnfreezePartialTokens([account_A], [AMOUNT]),
        ).to.be.revertedWithCustomError(
          erc1410Facet,
          'NotAllowedInMultiPartitionMode',
        );
