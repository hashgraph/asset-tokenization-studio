    ...
    describe('single partition', () => {
        ...
        describe('mint', () => {
            ...
            it('GIVEN a paused token WHEN attempting to mint TokenIsPaused error', async () => {
                ...
                await expect(
                    erc3643Facet.mint(signer_A.address, AMOUNT)
            ...
            it('GIVEN a max supply WHEN mint more than the max supply THEN transaction fails with MaxSupplyReached', async () => {
                await expect(
                    erc3643Facet
                        .connect(signer_A)
                        .mint(signer_E.address, MAX_SUPPLY + 1)
            ...
            it('GIVEN non kyc account WHEN mint THEN transaction reverts with InvalidKycStatus', async () => {
                ...
                await expect(
                    erc3643Facet.mint(signer_E.address, AMOUNT)
        ...
        describe('burn', () => {
            ...
            it('GIVEN a paused token WHEN attempting to burn TokenIsPaused error', async () => {
                ...
                await expect(
                    erc3643Facet.burn(signer_A.address, AMOUNT)
        ...
        describe('Freeze', () => {
            ...
            it('GIVEN a valid address WHEN setAddressFrozen AND blacklist THEN address should be added (freeze) and removed (unfreeze) from control list', async () => {
                await expect(
                    freezeFacet.setAddressFrozen(signer_B.address, true)
                )
                    .to.emit(freezeFacet, 'AddressFrozen')
                    .withArgs(signer_B.address, true, signer_A.address)

                let isInControlList = await controlList.isInControlList(
                    signer_B.address
                )
                expect(isInControlList).to.equal(true)
                await expect(
                    freezeFacet.setAddressFrozen(signer_B.address, false)
                )
                    .to.emit(freezeFacet, 'AddressFrozen')
                    .withArgs(signer_B.address, false, signer_A.address)
                isInControlList = await controlList.isInControlList(
                    signer_B.address
                )
            ...
            it('GIVEN a valid address WHEN setAddressFrozen AND whitelist THEN address should be removed (freeze) and added (unfreeze) to control list', async () => {
                ...
                const newFreezeFacet = await ethers.getContractAt(
                    'FreezeFacet',
                ...
                await expect(
                    newFreezeFacet.setAddressFrozen(signer_B.address, true)
                )
                    .to.emit(newFreezeFacet, 'AddressFrozen')
                    .withArgs(signer_B.address, true, signer_A.address)

                let isInControlList = await newControlList.isInControlList(
                    signer_B.address
                )
                expect(isInControlList).to.equal(false)
                await expect(
                    newFreezeFacet.setAddressFrozen(signer_B.address, false)
                )
                    .to.emit(newFreezeFacet, 'AddressFrozen')
                    .withArgs(signer_B.address, false, signer_A.address)
                isInControlList = await newControlList.isInControlList(
                    signer_B.address
                )
                expect(isInControlList).to.equal(true)
            })
            ...
            it('GIVEN a valid address WHEN attempting to freezePartialTokens THEN transactions succeed', async () => {
                ...
                await expect(
                    freezeFacet.freezePartialTokens(signer_E.address, amount)
                )
                    .to.emit(freezeFacet, 'TokensFrozen')
                    .withArgs(signer_E.address, amount, DEFAULT_PARTITION)
                expect(
                    await freezeFacet.getFrozenTokens(signer_E.address)
                ).to.be.equal(amount)
                expect(
                    await erc1410Facet.balanceOf(signer_E.address)
                ).to.be.equal(0)
            })
            ...
            it('GIVEN a freeze amount greater than balance WHEN attempting to freezePartialTokens THEN transactions revert with InsufficientBalance error', async () => {
                ...
                await expect(
                    freezeFacet.freezePartialTokens(
                        signer_E.address,
                        amount + 1
                    )
            ...
            it('GIVEN a valid address WHEN attempting to unfreezePartialTokens THEN transactions succeed', async () => {
                ...
                await expect(
                    freezeFacet.unfreezePartialTokens(signer_E.address, amount)
                )
            ...
            it('GIVEN a freeze amount greater than balance WHEN attempting to unfreezePartialTokens THEN transactions revert with InsufficientFrozenBalance error', async () => {
                ...
                await freezeFacet.freezePartialTokens(signer_E.address, amount)
                await expect(
                    freezeFacet.unfreezePartialTokens(
                        signer_E.address,
                        amount + 1
                    )
                )
        ...
        describe('Identity', () => {
            ...
            it('GIVEN non verified account with balance WHEN transfer THEN reverts with AddressNotVerified', async () => {
                ...
                await erc20Facet
                    .connect(signer_E)
                    .approve(signer_D.address, MAX_UINT256)
                await erc1410Facet
                    .connect(signer_E)
                    .authorizeOperator(signer_D.address)
                ...
                await expect(
                    erc20Facet
                        .connect(signer_E)
                        .transfer(signer_D.address, AMOUNT)
                ).to.be.revertedWithCustomError(
                    erc3643Facet,
                    'AddressNotVerified'
                )
                ...
                await expect(
                    erc3643Facet
                        .connect(signer_E)
                        .batchTransfer([signer_D.address], [AMOUNT])
                ).to.be.revertedWithCustomError(
                    erc3643Facet,
                    'AddressNotVerified'
                )
            ...
            it('GIVEN non verified account WHEN issue THEN reverts with AddressNotVerified', async () => {
                ...
                // Issue
                await expect(
                    erc3643Facet.batchMint([signer_E.address], [AMOUNT])
                ).to.be.revertedWithCustomError(
                    erc3643Facet,
                    'AddressNotVerified'
                )
            ...
            it('GIVEN non verified account WHEN redeem THEN reverts with AddressNotVerified', async () => {
                ...
                await expect(
                    erc1594Facet
                        .connect(signer_D)
                        .redeemFrom(signer_E.address, AMOUNT, EMPTY_HEX_BYTES)
                ).to.be.revertedWithCustomError(
                    erc3643Facet,
                    'AddressNotVerified'
                )
        ...
        describe('ERC3643 canTransfer Compliance Integration', () => {
            it('GIVEN ComplianceMock.canTransfer returns false THEN transfers fail with ComplianceNotAllowed', async () => {
                ...
                await expect(
                    erc20Facet
                        .connect(signer_E)
                        .transfer(signer_D.address, AMOUNT / 2)
                ).to.be.revertedWithCustomError(
                    erc3643Facet,
            ...
            it('GIVEN zero address compliance THEN transfers succeed without compliance checks', async () => {
                ...
                await expect(
                    erc20NoCompliance.transfer(signer_D.address, AMOUNT / 2)
                ).to.not.be.reverted
            })
        })
        ...
        describe('Compliance', () => {
            it('GIVEN an initialized token WHEN updating the compliance THEN setCompliance emits ComplianceAdded with updated compliance', async () => {
                ...
                expect(
                    await erc3643Facet.setCompliance(newComplianceMock.address)
                )
            ...
            it('GIVEN a successful transfer THEN transferred is called in compliance contract', async () => {
                ...
                await erc20Facet
                    .connect(signer_D)
                    .approve(signer_E.address, MAX_UINT256)
                await erc20Facet
                    .connect(signer_E)
                    .approve(signer_D.address, MAX_UINT256)
                await erc1410Facet
                ...
                await erc1410Facet
                    .connect(signer_D)
                    .authorizeOperator(signer_E.address)
                // Issue
                await erc1410Facet.issueByPartition({
                    partition: DEFAULT_PARTITION,
                    tokenHolder: signer_E.address,
                ...
                await erc20Facet
                    .connect(signer_E)
                    .transferFrom(signer_D.address, signer_E.address, AMOUNT)
                transfersCounter++
                await erc20Facet
                    .connect(signer_E)
                    .transfer(signer_D.address, AMOUNT)
                ...
                await erc1594Facet
                    .connect(signer_E)
                    .transferWithData(signer_D.address, AMOUNT, EMPTY_HEX_BYTES)
                transfersCounter++
                await erc1594Facet
                    .connect(signer_E)
                    .transferFromWithData(
            ...
            it('GIVEN a failed canTransfer call THEN transaction reverts with custom error', async () => {
                ...
                try {
                    await erc20Facet
                        .connect(signer_E)
                        .approve(signer_D.address, AMOUNT)
                } catch (err: any) {
                    caught = err
                }
                const returnedSelector = (caught.data as string).slice(0, 10)
            ...
            it('GIVEN ComplianceMock::canTransfer returns false THEN operations fail with ComplianceNotAllowed', async () => {
                ...
                await expect(
                    erc20Facet
                        .connect(signer_E)
                        .increaseAllowance(signer_D.address, AMOUNT)
                ).to.be.revertedWithCustomError(
                    erc3643Facet,
                    'ComplianceNotAllowed'
                )
                ...
                await expect(
                    erc1410Facet
                        .connect(signer_E)
                        .revokeOperator(signer_D.address)
                ).to.be.revertedWithCustomError(
                    erc3643Facet,
                    'ComplianceNotAllowed'
                )
        ...
        describe('Batch Operations', () => {
            describe('batchMint', () => {
                it('GIVEN an account with issuer role WHEN batchMint THEN transaction succeeds and balances are updated', async () => {
                    ...
                    const finalBalanceD = await erc1410SnapshotFacet.balanceOf(
                        signer_D.address
                    )
                    const finalBalanceE = await erc1410SnapshotFacet.balanceOf(
                        signer_E.address
                    )
            ...
            describe('batchTransfer', () => {
                ...
                it('GIVEN a valid sender WHEN batchTransfer THEN transaction succeeds and balances are updated', async () => {
                    ...
                    const initialBalanceF =
                        await erc1410SnapshotFacet.balanceOf(signer_F.address)
                    const initialBalanceD =
                        await erc1410SnapshotFacet.balanceOf(signer_D.address)
                    ...
                    const finalBalanceSender =
                        await erc1410SnapshotFacet.balanceOf(signer_E.address)
                    const finalBalanceF = await erc1410SnapshotFacet.balanceOf(
                        signer_F.address
                    )
                    const finalBalanceD = await erc1410SnapshotFacet.balanceOf(
                        signer_D.address
            ...
            describe('batchBurn', () => {
                ...
                beforeEach(async () => {
                    ...
                    await erc20Facet
                        .connect(signer_D)
                        .approve(signer_A.address, burnAmount)
                    await erc20Facet
                        .connect(signer_E)
                        .approve(signer_A.address, burnAmount)
                })
            ...
            describe('batchSetAddressFrozen', () => {
                ...
                it('GIVEN a FREEZE_MANAGER WHEN batchSetAddressFrozen with true THEN transfers from those addresses fail', async () => {
                    ...
                    await expect(
                        erc20Facet
                            .connect(signer_D)
                            .transfer(signer_A.address, transferAmount)
                    ).to.be.revertedWithCustomError(
                        controlList,
                        'AccountIsBlocked'
                    )
            ...
            describe('batchFreezePartialTokens', () => {
                ...
                it('GIVEN ATS_ROLES._FREEZE_MANAGER_ROLE WHEN batchFreezePartialTokens THEN tokens are frozen successfully', async () => {
                    ...
                    const initialFrozenD = await freezeFacet.getFrozenTokens(
                        signer_D.address
                    )
                    const initialFrozenE = await freezeFacet.getFrozenTokens(
                        signer_E.address
                    )
                    ...
                    const finalFrozenD = await freezeFacet.getFrozenTokens(
                        signer_D.address
                    )
                    const finalFrozenE = await freezeFacet.getFrozenTokens(
                        signer_E.address
                    )
        ...
        describe('Agent', () => {
            ...
            it('GIVEN a user with the agent role WHEN performing actions using ERC-1400 methods succeeds', async () => {
                ...
                await expect(
                    erc1644Facet
                        .connect(signer_B)
                        .controllerRedeem(signer_E.address, amount, '0x', '0x')
            ...
            it('GIVEN a user with the agent role WHEN performing actions using ERC-3643 methods succeeds', async () => {
                ...
                await expect(
                    freezeFacet
                        .connect(signer_B)
                        .freezePartialTokens(signer_E.address, amount)
                )
                    .to.emit(freezeFacet, 'TokensFrozen')
                    .withArgs(signer_E.address, amount, DEFAULT_PARTITION)
                await expect(
                    freezeFacet
                        .connect(signer_B)
                        .unfreezePartialTokens(signer_E.address, amount)
                )
                    .to.emit(freezeFacet, 'TokensUnfrozen')
                    .withArgs(signer_E.address, amount, DEFAULT_PARTITION)
                await expect(
                ...
                await expect(
                    freezeFacet
                        .connect(signer_B)
                        .setAddressFrozen(signer_E.address, true)
                )
                    .to.emit(freezeFacet, 'AddressFrozen')
        ...
        describe('AccessControl', () => {
            ...
            it('GIVEN an account without TREX_OWNER role WHEN setIdentityRegistry THEN transaction fails with AccountHasNoRole', async () => {
                ...
                await expect(
                    erc3643Facet
                        .connect(signer_C)
                        .setIdentityRegistry(identityRegistryMock.address)
                ).to.be.rejectedWith('AccountHasNoRole')
            })
            it('GIVEN an account without TREX_OWNER role WHEN setCompliance THEN transaction fails with AccountHasNoRole', async () => {
                ...
                await expect(
                    erc3643Facet
                        .connect(signer_C)
                        .setCompliance(complianceMock.address)
                ).to.be.rejectedWith('AccountHasNoRole')
            })
            ...
            it('GIVEN an account without FREEZE MANAGER role WHEN freezePartialTokens THEN transaction fails with AccountHasNoRole', async () => {
                await expect(
                    freezeFacet
                        .connect(signer_C)
                        .freezePartialTokens(signer_A.address, 10)
                ).to.be.rejectedWith('AccountHasNoRole')
            })
            ...
            it('GIVEN an account without FREEZE MANAGER role WHEN unfreezePartialTokens THEN transaction fails with AccountHasNoRole', async () => {
                await expect(
                    freezeFacet
                        .connect(signer_C)
                        .unfreezePartialTokens(signer_A.address, 10)
                ).to.be.rejectedWith('AccountHasNoRole')
            })
            ...
            it('GIVEN an account without FREEZE MANAGER role WHEN setAddressFrozen THEN transaction fails with AccountHasNoRole', async () => {
                await expect(
                    freezeFacet
                        .connect(signer_C)
                        .setAddressFrozen(signer_A.address, true)
                ).to.be.rejectedWith('AccountHasNoRole')
            })
            ...
            it('GIVEN an account without admin role WHEN addAgent THEN transaction fails with AccountHasNoRole', async () => {
                // add agent fails
                await expect(
                    erc3643Facet.connect(signer_C).addAgent(signer_A.address)
                ).to.be.rejectedWith('AccountHasNoRole')
            })
        })
        ...
        describe('Paused', () => {
            ...
            it('GIVEN a paused token WHEN attempting to update name or symbol THEN transactions revert with TokenIsPaused error', async () => {
                ...
                await expect(
                    erc3643Facet.setIdentityRegistry(
                        identityRegistryMock.address
                    )
                ).to.be.rejectedWith('TokenIsPaused')
                await expect(
                    erc3643Facet.setCompliance(complianceMock.address)
                ).to.be.rejectedWith('TokenIsPaused')
            })
        })
        ...
        describe('Recovery', () => {
            ...
            it('GIVEN lost wallet WHEN calling recoveryAddress THEN normal balance and freeze balance and status is successfully transferred', async () => {
                ...
                const frozenBalanceE = await freezeFacet.getFrozenTokens(
                    signer_E.address
                )
                const frozenBalanceB = await freezeFacet.getFrozenTokens(
                    signer_B.address
                )
                const controlListStatusE = await controlList.isInControlList(
                    signer_E.address
                )
                const controlListStatusB = await controlList.isInControlList(
                    signer_B.address
                )
                const isRecovered = await erc3643Facet.isAddressRecovered(
                    signer_E.address
                )
            ...
            it('GIVEN lost wallet WHEN calling recovery using a previously recovered address THEN recovered status is set to false', async () => {
                ...
                const isRecoveredC = await erc3643Facet.isAddressRecovered(
                    signer_C.address
                )
            ...
            it('GIVEN a recovered address THEN operations should fail', async () => {
                ...
                await erc1410Facet
                    .connect(signer_C)
                    .authorizeOperator(signer_A.address)
                await erc1410Facet
                    .connect(signer_C)
                ...
                await expect(
                    erc20Facet.transfer(signer_C.address, amount)
                ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                await expect(
                ...
                await expect(
                    erc3643Facet.batchTransfer([signer_C.address], [amount])
                ).to.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                // Allowance
                // 1 - Operator
                await expect(
                ...
                await expect(
                    erc20Facet
                        .connect(signer_C)
                        .approve(signer_A.address, amount)
                ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                await expect(
                    erc1410Facet
                        .connect(signer_C)
                ...
                await expect(
                    erc20Facet.approve(signer_C.address, amount)
                ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                await expect(
                    erc1410Facet.authorizeOperator(signer_C.address)
                ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                await expect(
                ...
                await expect(
                    erc1594Facet
                        .connect(signer_C)
                        .redeemFrom(signer_A.address, amount, EMPTY_HEX_BYTES)
                ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                // 2 - From
                await expect(
                ...
                await expect(
                    erc3643Facet.mint(signer_C.address, amount)
                ).to.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                await expect(
                    erc3643Facet.batchMint([signer_C.address], [amount])
                ).to.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                // Locks
                await expect(
                    lockFacet.lock(amount, signer_C.address, MAX_UINT256)
                ).to.be.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
                await expect(
                ...
                let canTransfer = await erc1594Facet
                    .connect(signer_C)
                    .canTransfer(signer_A.address, amount, EMPTY_HEX_BYTES)
                ...
                // Freeze
                await expect(
                    freezeFacet.freezePartialTokens(signer_C.address, amount)
                ).to.revertedWithCustomError(erc3643Facet, 'WalletRecovered')
            })
        })
    })
    ...
    describe('multi partition', () => {
        ...
        it('GIVEN an account with issuer role WHEN mint THEN transaction fails with NotAllowedInMultiPartitionMode', async () => {
            ...
            await expect(
                erc3643Facet
                    .connect(signer_C)
                    .mint(signer_D.address, 2 * BALANCE_OF_C_ORIGINAL)
            ).to.be.revertedWithCustomError(
                erc1410Facet,
        ...
        it('GIVEN an initialized token WHEN burning THEN transaction fails with NotAllowedInMultiPartitionMode', async () => {
            ...
            await expect(
                erc3643Facet
                    .connect(signer_C)
                    .burn(signer_C.address, 2 * BALANCE_OF_C_ORIGINAL)
            ).to.be.revertedWithCustomError(
                erc1410Facet,
                'NotAllowedInMultiPartitionMode'
            )
        ...
        describe('Batch operations', () => {
            it('GIVEN an single partition token WHEN batchTransfer THEN transaction fails with NotAllowedInMultiPartitionMode', async () => {
                await expect(
                    erc3643Facet.batchTransfer([signer_A.address], [AMOUNT])
                ).to.be.revertedWithCustomError(
                    erc1410Facet,
                    'NotAllowedInMultiPartitionMode'
                )
            ...
            it('GIVEN an single partition token WHEN batchMint THEN transaction fails with NotAllowedInMultiPartitionMode', async () => {
                await expect(
                    erc3643Facet.batchMint([signer_A.address], [AMOUNT])
                ).to.be.revertedWithCustomError(
                    erc1410Facet,
                    'NotAllowedInMultiPartitionMode'
                )
            ...
            it('GIVEN an single partition token WHEN batchBurn THEN transaction fails with NotAllowedInMultiPartitionMode', async () => {
                await expect(
                    erc3643Facet.batchBurn([signer_A.address], [AMOUNT])
                ).to.be.revertedWithCustomError(
                    erc1410Facet,
                    'NotAllowedInMultiPartitionMode'
                )
