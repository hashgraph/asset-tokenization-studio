...
describe('ExternalControlList Management Tests', () => {
    ...
    async function deployTokenSecurity() {
        ...
        externalWhitelistMock1 = await (
            await ethers.getContractFactory('MockedWhitelist', signer_A)
        ).deploy()
        await externalWhitelistMock1.deployed()

        externalBlacklistMock1 = await (
            await ethers.getContractFactory('MockedBlacklist', signer_A)
        ).deploy()
        await externalBlacklistMock1.deployed()

        externalWhitelistMock2 = await (
            await ethers.getContractFactory('MockedWhitelist', signer_A)
        ).deploy()
