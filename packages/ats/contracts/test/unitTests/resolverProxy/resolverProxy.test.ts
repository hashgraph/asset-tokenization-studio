...
describe('ResolverProxy Tests', () => {
    ...
    async function deployContracts() {
        ...
        diamondFacet = await (
            await ethers.getContractFactory('DiamondFacet', signer_A)
        ).deploy()
        accessControlImpl = await (
            await ethers.getContractFactory('AccessControlFacet', signer_A)
        ).deploy()
        pauseImpl = await (
            await ethers.getContractFactory('PauseFacet', signer_A)
        ).deploy()
    }
