...
describe("Kyc Tests", () => {
  ...
  describe("Paused", () => {
    ...
    it("GIVEN a paused Token WHEN grantKyc THEN transaction fails with TokenIsPaused", async () => {
      await expect(
        kycFacet.grantKyc(signer_B.address, _VC_ID, _VALID_FROM, _VALID_TO, signer_C.address),
    ...
    it("GIVEN a paused Token WHEN revokeKyc THEN transaction fails with TokenIsPaused", async () => {
      await expect(kycFacet.revokeKyc(signer_B.address)).to.be.revertedWithCustomError(kycFacet, "TokenIsPaused");
    });
  });
  ...
  describe("Access Control", () => {
    it("GIVEN a non Kyc account WHEN grantKyc THEN transaction fails with AccountHasNoRole", async () => {
      await expect(
        kycFacet.connect(signer_C).grantKyc(signer_B.address, _VC_ID, _VALID_FROM, _VALID_TO, signer_C.address),
    ...
    it("GIVEN a paused Token WHEN revokeKyc THEN transaction fails with AccountHasNoRole", async () => {
      await expect(kycFacet.connect(signer_C).revokeKyc(signer_B.address)).to.be.rejectedWith("AccountHasNoRole");
    });
    ...
    it("GIVEN an account without ATS_ROLES._INTERNAL_KYC_MANAGER_ROLE WHEN activating internal kyc THEN it reverts with AccountHasNoRole", async () => {
      await expect(kycFacet.connect(signer_C).activateInternalKyc()).to.be.rejectedWith("AccountHasNoRole");
    });
    ...
    it("GIVEN an account without ATS_ROLES._INTERNAL_KYC_MANAGER_ROLE WHEN deactivating internal kyc THEN it reverts with AccountHasNoRole", async () => {
      await expect(kycFacet.connect(signer_C).deactivateInternalKyc()).to.be.rejectedWith("AccountHasNoRole");
    });
  });
  ...
  describe("Kyc Valid Status Modifier Tests (onlyValidKycStatus)", () => {
    it("GIVEN internal KYC is deactivated WHEN grantKyc is called THEN KYC status is updated successfully", async () => {
      ...
      const KycStatusFor_B_Before = await kycFacet.getKycStatusFor(signer_B.address);

      await kycFacet.grantKyc(signer_B.address, _VC_ID, _VALID_FROM, _VALID_TO, signer_C.address);

      const KycStatusFor_B_After = await kycFacet.getKycStatusFor(signer_B.address);
    ...
    it("GIVEN internal KYC is deactivated AND external KYC has different status WHEN grantKyc is called THEN it reverts with InvalidKycStatus", async () => {
      await kycFacet.deactivateInternalKyc();
      await externalKycListManagement.addExternalKycList(externalKycListMock.address);
  ...
  describe("Kyc Wrong input data", () => {
    it("GIVEN account ZERO WHEN grantKyc THEN transaction fails with ZeroAddressNotAllowed", async () => {
      await expect(
        kycFacet.grantKyc(ADDRESS_ZERO, _VC_ID, _VALID_FROM, _VALID_TO, signer_C.address),
    ...
    it("GIVEN account ZERO WHEN revokeKyc THEN transaction fails with ZeroAddressNotAllowed", async () => {
      await expect(kycFacet.revokeKyc(ADDRESS_ZERO)).to.be.revertedWithCustomError(kycFacet, "ZeroAddressNotAllowed");
    });

    it("GIVEN wrong Valid From Date WHEN grantKyc THEN transaction fails with InvalidDates", async () => {
      await expect(
        kycFacet.grantKyc(signer_B.address, _VC_ID, _VALID_TO + 1, _VALID_TO, signer_C.address),
    ...
    it("GIVEN wrong Valid To Date WHEN grantKyc THEN transaction fails with InvalidDates", async () => {
      await expect(
        kycFacet.grantKyc(signer_B.address, _VC_ID, _VALID_FROM, currentTimestamp - 1, signer_C.address),
      ).to.be.revertedWithCustomError(kycFacet, "InvalidDates");
    });

    it("GIVEN wrong issuer WHEN grantKyc THEN transaction fails with AccountIsNotIssuer", async () => {
      await expect(
        kycFacet.grantKyc(signer_B.address, _VC_ID, _VALID_FROM, _VALID_TO, signer_D.address),
  ...
  describe("Kyc OK", () => {
    it("GIVEN a VC WHEN grantKyc THEN transaction succeed", async () => {
      ...
      const [kycAccounts, kycAccountsData_After] = await kycFacet.getKycAccountsData(1, 0, 1);
    ...
    it("GIVEN a VC WHEN revokeKyc THEN transaction succeed", async () => {
      ...
      const KYCStatusFor_B_After = await kycFacet.getKycStatusFor(signer_B.address);
      const KYC_Count_After = await kycFacet.getKycAccountsCount(1);
      const [kycAccounts, kycAccountsData] = await kycFacet.getKycAccountsData(1, 0, 100);
    ...
    it("Check Kyc status after expiration", async () => {
      ...
      const KYCStatusFor_B_After_Grant = await kycFacet.getKycStatusFor(signer_B.address);

      await timeTravelFacet.changeSystemTimestamp(_VALID_TO + 1);

      const KYCStatusFor_B_After_Expiration = await kycFacet.getKycStatusFor(signer_B.address);
    ...
    it("Check Kyc status after issuer removed", async () => {
      ...
      const KYCStatusFor_B_After_Cancelling_Issuer = await kycFacet.getKycStatusFor(signer_B.address);
    ...
    it("Check Kyc status after issuer revokes VC", async () => {
      ...
      const KYCFor_B_After_Revoking_VC = await kycFacet.getKycStatusFor(signer_B.address);
