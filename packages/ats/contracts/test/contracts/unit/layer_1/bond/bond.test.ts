...
describe.only("Bond Tests", () => {
  ...
  describe("Bond Read - Ordered List", () => {
    ...
    let kpiBondFacet: any; // BondUSAKpiLinkedRateFacetTimeTravel type
    let kpiBondReadFacet: BondUSAReadFacet;
    let kpiTimeTravelFacet: TimeTravel;
    let kpiCurrentBlockTimestamp: number;

    async function deployKpiFixture() {
      kpiCurrentBlockTimestamp = await getDltTimestamp();
      const startDate = kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY;
      const matDate = startDate + TIME_PERIODS_S.YEAR;

      const { diamond, deployer } = await deployBondKpiLinkedRateTokenFixture({
        bondDataParams: {
          bondDetails: {
      ...
      const bFacet = await ethers.getContractAt("BondUSAKpiLinkedRateFacetTimeTravel", kpiDiamond.address);
      const brFacet = await ethers.getContractAt("BondUSAReadFacet", kpiDiamond.address);
      const accessControlFacet = await ethers.getContractAt("AccessControl", kpiDiamond.address);
      const ttFacet = await ethers.getContractAt("TimeTravelFacet", kpiDiamond.address);

      // Grant corporate action role
      await accessControlFacet.grantRole(ATS_ROLES._CORPORATE_ACTION_ROLE, deployer.address);

      return { bondFacet: bFacet, bondReadFacet: brFacet, timeTravelFacet: ttFacet };
    }

    beforeEach(async () => {
      const { bondFacet: bf, bondReadFacet: brf, timeTravelFacet: ttf } = await loadFixture(deployKpiFixture);
      kpiBondFacet = bf;
      kpiBondReadFacet = brf;
      kpiTimeTravelFacet = ttf;
    });

    describe("getCouponsOrderedListTotal", () => {
      it("should return 0 when no coupons have been created", async () => {
        const total = await kpiBondReadFacet.getCouponsOrderedListTotal();
      ...
      it("should return the correct count when coupons exist", async () => {
        ...
        const coupon1 = {
          recordDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY,
          executionDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 2,
          rate: 0,
          rateDecimals: 0,
          startDate: kpiCurrentBlockTimestamp,
          endDate: kpiCurrentBlockTimestamp + YEAR_SECONDS,
          fixingDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY,
          rateStatus: 0,
        };

        await kpiBondFacet.setCoupon(coupon1);

        const coupon2 = {
          recordDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 3,
          executionDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 4,
          rate: 0,
          rateDecimals: 0,
          startDate: kpiCurrentBlockTimestamp,
          endDate: kpiCurrentBlockTimestamp + YEAR_SECONDS,
          fixingDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 3,
          rateStatus: 0,
        };

        await kpiBondFacet.setCoupon(coupon2);

        const coupon3 = {
          recordDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 5,
          executionDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 6,
          rate: 0,
          rateDecimals: 0,
          startDate: kpiCurrentBlockTimestamp,
          endDate: kpiCurrentBlockTimestamp + YEAR_SECONDS,
          fixingDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 5,
          rateStatus: 0,
        };

        await kpiBondFacet.setCoupon(coupon3);

        // Move time forward past all fixing dates
        await kpiTimeTravelFacet.changeSystemTimestamp(kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 6);
    ...
    describe("getCouponFromOrderedListAt", () => {
      it("should return 0 for invalid position when no coupons exist", async () => {
        const couponId = await kpiBondReadFacet.getCouponFromOrderedListAt(0);
      ...
      it("should return 0 for position beyond the list", async () => {
        ...
        await kpiBondFacet.setCoupon(coupon);

        // Move time forward past fixing date
        await kpiTimeTravelFacet.changeSystemTimestamp(kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 2);

        // Try to get position 1 (second item) when only 1 exists (index 0)
        const couponId = await kpiBondReadFacet.getCouponFromOrderedListAt(1);
        expect(couponId).to.equal(0);
      });
      ...
      it("should return correct coupon ID at specific position", async () => {
        ...
        const tx1 = await kpiBondFacet.setCoupon(coupon1);
        await tx1.wait();
        ...
        const tx2 = await kpiBondFacet.setCoupon(coupon2);
        await tx2.wait();
        ...
        const tx3 = await kpiBondFacet.setCoupon(coupon3);
        await tx3.wait();
        ...
        const couponId0 = await kpiBondReadFacet.getCouponFromOrderedListAt(0);
        expect(couponId0).to.equal(1);

        // Get coupon at position 1 (second coupon)
        const couponId1 = await kpiBondReadFacet.getCouponFromOrderedListAt(1);
        expect(couponId1).to.equal(2);

        // Get coupon at position 2 (third coupon)
        const couponId2 = await kpiBondReadFacet.getCouponFromOrderedListAt(2);
    ...
    describe("getCouponsOrderedList", () => {
      it("should return empty array when no coupons exist", async () => {
        const coupons = await kpiBondReadFacet.getCouponsOrderedList(0, 10);
      ...
      it("should return paginated results correctly", async () => {
        ...
        for (let i = 0; i < 5; i++) {
          const coupon = {
            recordDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * (i * 2 + 1),
            executionDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * (i * 2 + 2),
            rate: 0,
            rateDecimals: 0,
            startDate: kpiCurrentBlockTimestamp,
            endDate: kpiCurrentBlockTimestamp + YEAR_SECONDS,
            fixingDate: kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * (i * 2 + 1),
            rateStatus: 0,
          };
        ...
        // Move time forward past all fixing dates
        await kpiTimeTravelFacet.changeSystemTimestamp(kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 11);

        // Get first page (2 items)
        const page1 = await kpiBondReadFacet.getCouponsOrderedList(0, 2);
        expect(page1).to.be.an("array").with.lengthOf(2);
        expect(page1[0]).to.equal(1);
        expect(page1[1]).to.equal(2);

        // Get second page (2 items)
        const page2 = await kpiBondReadFacet.getCouponsOrderedList(1, 2);
        ...
        const page3 = await kpiBondReadFacet.getCouponsOrderedList(2, 2);
        expect(page3).to.be.an("array").with.lengthOf(1);
        expect(page3[0]).to.equal(5);

        // Get page beyond available data
        const page4 = await kpiBondReadFacet.getCouponsOrderedList(3, 2);
      ...
      it("should handle single item per page", async () => {
        ...
        // Move time forward past all fixing dates
        await kpiTimeTravelFacet.changeSystemTimestamp(kpiCurrentBlockTimestamp + TIME_PERIODS_S.DAY * 7);

        // Get page 0 (first item)
        const page0 = await kpiBondReadFacet.getCouponsOrderedList(0, 1);
        expect(page0).to.be.an("array").with.lengthOf(1);
        expect(page0[0]).to.equal(1);

        // Get page 1 (second item)
        const page1 = await kpiBondReadFacet.getCouponsOrderedList(1, 1);
        expect(page1).to.be.an("array").with.lengthOf(1);
        expect(page1[0]).to.equal(2);

        // Get page 2 (third item)
        const page2 = await kpiBondReadFacet.getCouponsOrderedList(2, 1);
