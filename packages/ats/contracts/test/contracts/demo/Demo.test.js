...
describe("Demo RedSwam", () => {
  let businessLogicResolver, diamond, diamondLoupe, diamondCut, testFacet1, testFacet2;
  ...
  async function deployDiamond() {
    ...
    diamond = await hardhat_1.ethers
      ...
      .then(async (contract) => await contract.deploy(resolver, businessLogicKeys, diamondArgs));
  }
  ...
  it("Demo RedSwam", async () => {
    await (0, hardhat_network_helpers_1.loadFixture)(deployBusinessLogicResolver);
...
Deployed contracts:
    ...
    DiamondCutFacet: 
        ...
        selectors: ${JSON.stringify(await diamondCut.getStaticFunctionSelectors())},
    DiamondLoupeFacet: 
        address: ${await diamondLoupe.address},
        key: ${await diamondLoupe.getStaticResolverKey()},
        selectors: ${JSON.stringify(await diamondLoupe.getStaticFunctionSelectors())},
    TestFacet1: 
        address: ${await testFacet1.address},
        key: ${await testFacet1.getStaticResolverKey()},
        selectors: ${JSON.stringify(await testFacet1.getStaticFunctionSelectors())},
    TestFacet2: 
        address: ${await testFacet2.address},
        key: ${await testFacet2.getStaticResolverKey()},
        selectors: ${JSON.stringify(await testFacet2.getStaticFunctionSelectors())}`);
    ...
    await (0, chai_1.expect)(businessLogicResolver.registerBusinessLogics(businessLogicRegistries)).to.emit(
      businessLogicResolver,
      "BusinessLogicsRegistered",
    );
    (0, chai_1.expect)(await businessLogicResolver.getVersionStatus(1)).to.be.equal(VersionStatus.ACTIVATED);
    (0, chai_1.expect)(await businessLogicResolver.getLatestVersion()).to.be.equal(1);
    (0, chai_1.expect)(
      await businessLogicResolver.resolveLatestBusinessLogic(await diamondCut.getStaticResolverKey()),
    ).to.be.equal(await diamondCut.address);
    (0, chai_1.expect)(
      await businessLogicResolver.resolveLatestBusinessLogic(await diamondLoupe.getStaticResolverKey()),
    ).to.be.equal(await diamondLoupe.address);
    (0, chai_1.expect)(
      await businessLogicResolver.resolveLatestBusinessLogic(await testFacet1.getStaticResolverKey()),
    ).to.be.equal(await testFacet1.address);
    (0, chai_1.expect)(
      await businessLogicResolver.resolveLatestBusinessLogic(await testFacet2.getStaticResolverKey()),
    ).to.be.equal(await testFacet2.address);
    (0, chai_1.expect)(
      await businessLogicResolver.resolveBusinessLogicByVersion(await diamondCut.getStaticResolverKey(), 1),
    ).to.be.equal(await diamondCut.address);
    (0, chai_1.expect)(await businessLogicResolver.getBusinessLogicCount()).to.be.equal(4);
    (0, chai_1.expect)(await businessLogicResolver.getBusinessLogicKeys(0, 10)).to.be.deep.equal([
      await diamondCut.getStaticResolverKey(),
      await diamondLoupe.getStaticResolverKey(),
      await testFacet1.getStaticResolverKey(),
      await testFacet2.getStaticResolverKey(),
