...
export function externalPauseTests(getCtx: () => AssetMockCtx): void {
  describe("ExternalPause Tests", () => {
    ...
    beforeEach(async () => {
      ...
      await externalPauseMock1.setPaused(false, { gasLimit: GAS_LIMIT.default });
      await externalPauseMock2.setPaused(false, { gasLimit: GAS_LIMIT.default });
      await externalPauseMock3.setPaused(false, { gasLimit: GAS_LIMIT.default });

      await asset.addExternalPause(externalPauseMock1.target as string, { gasLimit: GAS_LIMIT.default });
      await asset.addExternalPause(externalPauseMock2.target as string, { gasLimit: GAS_LIMIT.default });
    });
