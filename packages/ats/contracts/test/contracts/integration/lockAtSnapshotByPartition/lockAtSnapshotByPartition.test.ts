...
import { deployEquityTokenFixture, deployAtsInfrastructureFixture, executeRbac, MAX_UINT256 } from "@test";
...
describe("LockAtSnapshotByPartition Tests", () => {
  ...
  describe("initializeLockAtSnapshotByPartition", () => {
    it("GIVEN a caller without DEFAULT_ADMIN_ROLE WHEN initializeLockAtSnapshotByPartition is called THEN it reverts with AccountHasNoRole", async () => {
      ...
      await expect(freshAsset.connect(signer_C).initializeLockAtSnapshotByPartition()).to.be.revertedWithCustomError(
        freshAsset,
        "AccountHasNoRole",
      );
    });
    ...
    it("GIVEN an already-initialised facet WHEN initializeLockAtSnapshotByPartition is called again THEN it reverts with FacetAlreadyRegistered", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await freshAsset.connect(infra.deployer).initializeLockAtSnapshotByPartition();
      await expect(
        freshAsset.connect(infra.deployer).initializeLockAtSnapshotByPartition(),
    ...
    it("GIVEN a caller with DEFAULT_ADMIN_ROLE WHEN initializeLockAtSnapshotByPartition is called THEN it emits LockAtSnapshotByPartitionInitialized", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await expect(freshAsset.connect(infra.deployer).initializeLockAtSnapshotByPartition()).to.emit(
        freshAsset,
