...
export function principalTests(getCtx: () => AssetMockCtx): void {
  describe("Principal Tests", () => {
    ...
    beforeEach(async () => {
      ...
      const initEffectiveDatetime = (await getDltTimestamp()) - 3600;
      await asset.forceSetNominalValue(
        DEFAULT_NOMINAL_VALUE,
        Number(DEFAULT_NOMINAL_DECIMALS),
        CURRENCY_ZERO,
        initEffectiveDatetime,
        true,
      );
    ...
    describe("getPrincipalFor", () => {
      ...
      it("GIVEN an updated nominal value WHEN getPrincipalFor THEN scales the numerator proportionally", async () => {
        ...
        await asset.connect(deployer).publishNominalValue(newNominalValue, (await getDltTimestamp()) - 60);
