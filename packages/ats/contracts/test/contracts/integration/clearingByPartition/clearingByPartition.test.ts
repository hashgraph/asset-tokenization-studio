...
export function clearingByPartitionTests(getCtx: () => AssetMockCtx): void {
  describe("ClearingByPartitionFacet Tests", () => {
    ...
    describe("approveClearingOperationByPartition", () => {
      ...
      it("GIVEN a pending redeem clearing WHEN approveClearingOperationByPartition THEN totalSupply drops, clearedAmount zero and RedeemedByPartition emitted", async () => {
        ...
        expect(await asset.balanceOf(signer_A.address)).to.equal(0);
        expect(await asset.getClearedAmountForByPartition(_DEFAULT_PARTITION, signer_A.address)).to.equal(
          balanceBefore,
        );
    ...
    describe("getClearingCountForByPartition", () => {
      it("GIVEN two pending redeem clearings WHEN getClearingCountForByPartition THEN returns 2", async () => {
        ...
        expect(
          await asset.getClearingCountForByPartition(
            _DEFAULT_PARTITION,
            signer_A.address,
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN approveClearingOperationByPartition THEN transaction fails with Deactivated", async () => {
        await expect(
          asset.connect(signer_A).approveClearingOperationByPartition({
            clearingOperationType: ClearingOperationType.Redeem,
            partition: _DEFAULT_PARTITION,
            tokenHolder: signer_A.address,
            clearingId: 1,
      ...
      it("GIVEN a deactivated asset WHEN cancelClearingOperationByPartition THEN transaction fails with Deactivated", async () => {
        await expect(
          asset.connect(signer_A).cancelClearingOperationByPartition({
            clearingOperationType: ClearingOperationType.Redeem,
            partition: _DEFAULT_PARTITION,
            tokenHolder: signer_A.address,
            clearingId: 1,
      ...
      it("GIVEN a deactivated asset WHEN reclaimClearingOperationByPartition THEN transaction fails with Deactivated", async () => {
        await expect(
          asset.connect(signer_A).reclaimClearingOperationByPartition({
            clearingOperationType: ClearingOperationType.Redeem,
            partition: _DEFAULT_PARTITION,
            tokenHolder: signer_A.address,
            clearingId: 1,
      ...
      it("GIVEN a deactivated asset WHEN clearingRedeemFromByPartition THEN transaction fails with Deactivated", async () => {
        await expect(
          asset.connect(signer_B).clearingRedeemFromByPartition(
            {
              clearingOperation: {
                partition: _DEFAULT_PARTITION,
                expirationTimestamp: EXPIRATION_TIMESTAMP,
    ...
    describe("initializeClearingByPartition", () => {
      it("GIVEN caller without DEFAULT_ADMIN_ROLE WHEN initializeClearingByPartition is called THEN AccountHasNoRole", async () => {
        await expect(asset.connect(unknownSigner).initializeClearingByPartition())
          .to.be.revertedWithCustomError(asset, "AccountHasNoRole")
      ...
      it("GIVEN already-initialised WHEN initializeClearingByPartition is called again THEN FacetAlreadyRegistered", async () => {
        await expect(asset.initializeClearingByPartition())
          ...
          .withArgs(RESOLVER_KEYS.clearingByPartition, 1);
      });
    });

    describe("initializeClearingByPartition event", () => {
      it("GIVEN a caller with DEFAULT_ADMIN_ROLE WHEN initializeClearingByPartition is called THEN emits ClearingByPartitionInitialized", async () => {
        await asset.forceFacetNotRegistered(RESOLVER_KEYS.clearingByPartition);
