  ...
  describe("Multi-partition enabled", () => {
    ...
    describe("Paused", () => {
      ...
      it("GIVEN a paused Token WHEN lock THEN transaction fails with TokenIsPaused", async () => {
        // lockByPartition with data fails
        await expect(asset.connect(signer_C).lock(_AMOUNT, signer_A.address, currentTimestamp)).to.be.rejectedWith(
          "TokenIsPaused",
        );
      });
      ...
      it("GIVEN a paused Token WHEN release THEN transaction fails with TokenIsPaused", async () => {
        // transfer from with data fails
        await expect(asset.connect(signer_C).release(1, signer_A.address)).to.be.rejectedWith("TokenIsPaused");
      });
    });
    ...
    describe("AccessControl", () => {
      ...
      it("GIVEN an account without LOCKER role WHEN lock THEN transaction fails with AccountHasNoRole", async () => {
        // add to list fails
        await expect(asset.connect(signer_D).lock(_AMOUNT, signer_A.address, currentTimestamp)).to.be.rejectedWith(
          "AccountHasNoRole",
        );
      });
    });
