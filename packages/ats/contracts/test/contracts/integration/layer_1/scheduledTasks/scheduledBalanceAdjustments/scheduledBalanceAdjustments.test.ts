...
import { dateToUnixTimestamp, ATS_ROLES, EQUITY_CONFIG_ID } from "@scripts";
...
describe("Scheduled BalanceAdjustments Tests", () => {
  ...
  describe("initializeScheduledBalanceAdjustment", () => {
    it("GIVEN a caller without DEFAULT_ADMIN_ROLE WHEN initializeScheduledBalanceAdjustment is called THEN it reverts with AccountHasNoRole", async () => {
      ...
      await expect(freshAsset.connect(signer_C).initializeScheduledBalanceAdjustment()).to.be.revertedWithCustomError(
        freshAsset,
        "AccountHasNoRole",
      );
    });
    ...
    it("GIVEN an already-initialised facet WHEN initializeScheduledBalanceAdjustment is called again THEN it reverts with FacetAlreadyRegistered", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await freshAsset.connect(infra.deployer).initializeScheduledBalanceAdjustment();
      await expect(
        freshAsset.connect(infra.deployer).initializeScheduledBalanceAdjustment(),
    ...
    it("GIVEN a caller with DEFAULT_ADMIN_ROLE WHEN initializeScheduledBalanceAdjustment is called THEN it emits ScheduledBalanceAdjustmentInitialized", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await expect(freshAsset.connect(infra.deployer).initializeScheduledBalanceAdjustment()).to.emit(
        freshAsset,
