...
describe("Equity Tests", () => {
  ...
  describe("Dividends", () => {
    ...
    it("GIVEN an account without corporateActions role WHEN setDividend THEN transaction fails with AccountHasNoRole", async () => {
      // set dividend fails
      await expect(asset.connect(signer_C).setDividend(dividendData)).to.be.rejectedWith("AccountHasNoRole");
    });
    ...
    it("GIVEN a paused Token WHEN setDividend THEN transaction fails with TokenIsPaused", async () => {
      ...
      // set dividend fails
      await expect(asset.connect(signer_C).setDividend(dividendData)).to.be.rejectedWith("TokenIsPaused");
    });
    ...
    it("GIVEN an account with corporateActions role WHEN setDividend THEN transaction succeeds", async () => {
      ...
      // check list members
      await expect(asset.getDividend(1000)).to.be.rejectedWith("WrongIndexForAction");
    ...
    describe("Cancel Dividend", () => {
      it("GIVEN an account without corporateActions role WHEN cancelDividend THEN transaction fails with AccountHasNoRole", async () => {
        ...
        await expect(asset.connect(signer_C).cancelDividend(1)).to.be.rejectedWith("AccountHasNoRole");
      });
      ...
      it("GIVEN a paused Token WHEN cancelDividend THEN transaction fails with TokenIsPaused", async () => {
        ...
        await expect(asset.connect(signer_B).cancelDividend(1)).to.be.rejectedWith("TokenIsPaused");
      });
  ...
  describe("Voting rights", () => {
    ...
    it("GIVEN an account without corporateActions role WHEN setVoting THEN transaction fails with AccountHasNoRole", async () => {
      // set voting fails
      await expect(asset.connect(signer_C).setVoting(votingData)).to.be.rejectedWith("AccountHasNoRole");
    });
    ...
    it("GIVEN a paused Token WHEN setVoting THEN transaction fails with TokenIsPaused", async () => {
      ...
      // set voting fails
      await expect(asset.connect(signer_C).setVoting(votingData)).to.be.rejectedWith("TokenIsPaused");
    });
    ...
    it("GIVEN voting created WHEN trying to get voting with wrong ID type THEN transaction fails with WrongIndexForAction", async () => {
      ...
      await expect(asset.getVoting(2)).to.be.rejectedWith("WrongIndexForAction");

      // Try to access voting details with wrong ID (getVotingFor has the modifier)
      await expect(asset.getVotingFor(2, signer_A.address)).to.be.rejectedWith("WrongIndexForAction");
    ...
    it("GIVEN dividends created WHEN trying to get dividend with wrong ID type THEN transaction fails with WrongIndexForAction", async () => {
      ...
      await expect(asset.getDividend(2)).to.be.rejectedWith("WrongIndexForAction");
      await expect(asset.getDividendFor(2, signer_A.address)).to.be.rejectedWith("WrongIndexForAction");
      await expect(asset.getDividendAmountFor(2, signer_A.address)).to.be.rejectedWith("WrongIndexForAction");
    });
    it("GIVEN an account without corporateActions role WHEN setVoting THEN transaction fails with AccountHasNoRole", async () => {
      // set dividend fails
      await expect(asset.connect(signer_C).setVoting(votingData)).to.be.rejectedWith("AccountHasNoRole");
    ...
    it("GIVEN an account with corporateActions role WHEN setVoting THEN transaction succeeds", async () => {
      ...
      // check list members
      // await expect(asset.getVoting(1000)).to.be.rejectedWith(
      //     'WrongIndexForAction'
      // )
    ...
    describe("Cancel Voting", () => {
      it("GIVEN an account without corporateActions role WHEN cancelVoting THEN transaction fails with AccountHasNoRole", async () => {
        ...
        await expect(asset.connect(signer_C).cancelVoting(1)).to.be.rejectedWith("AccountHasNoRole");
      });
      ...
      it("GIVEN a paused Token WHEN cancelVoting THEN transaction fails with TokenIsPaused", async () => {
        ...
        await expect(asset.connect(signer_B).cancelVoting(1)).to.be.rejectedWith("TokenIsPaused");
      });
  ...
  describe("Balance adjustments", () => {
    it("GIVEN an account without corporateActions role WHEN setScheduledBalanceAdjustment THEN transaction fails with AccountHasNoRole", async () => {
      // set dividend fails
      await expect(asset.connect(signer_C).setScheduledBalanceAdjustment(balanceAdjustmentData)).to.be.rejectedWith(
        "AccountHasNoRole",
      );
    });
    ...
    it("GIVEN balance adjustment created WHEN trying to get balance adjustment with wrong ID type THEN transaction fails with WrongIndexForAction", async () => {
      ...
      await expect(asset.getScheduledBalanceAdjustment(2)).to.be.rejectedWith("WrongIndexForAction");
    });
    ...
    it("GIVEN an account with corporateActions role WHEN setScheduledBalanceAdjustment THEN transaction succeeds", async () => {
      ...
      const listCount = await asset.getScheduledBalanceAdjustmentCount();
    ...
    describe("Cancel Scheduled Balance Adjustment", () => {
      it("GIVEN an account without corporateActions role WHEN cancelScheduledBalanceAdjustment THEN transaction fails with AccountHasNoRole", async () => {
        ...
        await expect(asset.connect(signer_C).cancelScheduledBalanceAdjustment(1)).to.be.rejectedWith(
          "AccountHasNoRole",
        );
      });
      ...
      it("GIVEN a paused Token WHEN cancelScheduledBalanceAdjustment THEN transaction fails with TokenIsPaused", async () => {
        ...
        await expect(asset.connect(signer_B).cancelScheduledBalanceAdjustment(1)).to.be.rejectedWith("TokenIsPaused");
      });
