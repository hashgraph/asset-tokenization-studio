...
import { ZERO, EMPTY_STRING, ATS_ROLES, DEFAULT_PARTITION, EQUITY_CONFIG_ID } from "@scripts";
...
describe("FreezeAtSnapshot Tests", () => {
  ...
  describe("initializeFreezeAtSnapshot", () => {
    it("GIVEN a caller without DEFAULT_ADMIN_ROLE WHEN initializeFreezeAtSnapshot is called THEN it reverts with AccountHasNoRole", async () => {
      ...
      await expect(freshAsset.connect(signer_C).initializeFreezeAtSnapshot()).to.be.revertedWithCustomError(
        freshAsset,
        "AccountHasNoRole",
      );
    });
    ...
    it("GIVEN an already-initialised facet WHEN initializeFreezeAtSnapshot is called again THEN it reverts with FacetAlreadyRegistered", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await freshAsset.connect(infra.deployer).initializeFreezeAtSnapshot();
      await expect(freshAsset.connect(infra.deployer).initializeFreezeAtSnapshot()).to.be.revertedWithCustomError(
        freshAsset,
    ...
    it("GIVEN a caller with DEFAULT_ADMIN_ROLE WHEN initializeFreezeAtSnapshot is called THEN it emits FreezeAtSnapshotInitialized", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await expect(freshAsset.connect(infra.deployer).initializeFreezeAtSnapshot()).to.emit(
        freshAsset,
