...
export function couponTests(getCtx: () => AssetMockCtx): void {
  describe("Coupon Tests", () => {
    ...
    beforeEach(async () => {
      ...
      await executeRbac(asset, [
        ...
        {
          role: ATS_ROLES.ROLE_INTERNAL_KYC_MANAGER,
          members: [signer_A.address],
        },
        {
          role: ATS_ROLES.ROLE_MATURITY_MANAGER,
          members: [signer_A.address],
        },
        {
    ...
    it("GIVEN no existing coupon WHEN cancelCoupon with invalid ID THEN transaction fails", async () => {
      ...
      await expect(asset.connect(signer_C).cancelCoupon(999)).to.be.revertedWithCustomError(
        asset,
        "WrongIndexForAction",
      );
    });
    ...
    it("GIVEN a non-coupon corporate action WHEN call with invalid index view methods THEN transaction fails with WrongActionType", async () => {
      ...
      await expect(asset.getCouponFor(999, signer_A.address)).to.be.revertedWithCustomError(
        asset,
        "WrongIndexForAction",
      );
    ...
    describe("overflow safety and precision invariants", () => {
      ...
      it("GIVEN a high-precision configuration that overflows the pre-fix balance·nominal·rate·period four-way product WHEN getCouponAmountFor THEN does not revert", async () => {
        ...
        const canonicalDenominator =
          10n ** (couponFor.decimals + BigInt(HIGH_NOMINAL_DECIMALS) + BigInt(HIGH_RATE_DECIMALS)) *
          BigInt(YEAR_SECONDS);
      ...
      it("GIVEN decimals + rateDecimals in [70, 77] WHEN getCouponAmountFor after record date THEN reverts with GreaterThanMaxUint256", async () => {
        ...
        // pow10(70) x 365 days approx 3.15 x 10^77 > MAX_UINT256 approx 1.16 x 10^77:
  ...
  describe("Coupon Fixed-Rate Variant Tests", () => {
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN cancelCoupon THEN transaction fails with Deactivated", async () => {
        await expect(asset.cancelCoupon(0)).to.be.revertedWithCustomError(asset, "Deactivated");
      });

      it("GIVEN a deactivated asset WHEN forceCancelCoupon THEN transaction fails with Deactivated", async () => {
        await expect(asset.forceCancelCoupon(0)).to.be.revertedWithCustomError(asset, "Deactivated");
      });
    });
