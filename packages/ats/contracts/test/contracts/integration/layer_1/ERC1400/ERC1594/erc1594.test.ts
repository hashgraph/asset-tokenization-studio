...
describe("ERC1594 Tests", () => {
  ...
  describe("Multi partition mode", () => {
    ...
    it("GIVEN an initialized contract WHEN trying to initialize it again THEN transaction fails with AlreadyInitialized", async () => {
      await expect(asset.initialize_ERC1594()).to.be.rejectedWith("AlreadyInitialized");
    });
  ...
  describe("Single partition mode", () => {
    ...
    describe("Cap", () => {
      it("GIVEN a max supply WHEN issue more than the max supply THEN transaction fails with MaxSupplyReached", async () => {
        // add to list fails
        await expect(asset.connect(signer_A).issue(signer_E.address, MAX_SUPPLY + 1, DATA)).to.be.rejectedWith(
          "MaxSupplyReached",
        );
      });
    });
    ...
    describe("ControlList", () => {
      it("GIVEN blocked accounts (sender, to, from) WHEN transfer THEN transaction fails with AccountIsBlocked", async () => {
        ...
        // transfer with data fails
        await expect(asset.connect(signer_C).transferWithData(signer_D.address, AMOUNT, DATA)).to.be.rejectedWith(
          "AccountIsBlocked",
        );
      ...
      it("GIVEN blocked accounts (sender, from) WHEN redeem THEN transaction fails with AccountIsBlocked", async () => {
        ...
        await expect(asset.connect(signer_C).redeem(AMOUNT, DATA)).to.be.rejectedWith("AccountIsBlocked");

        // redeem from with data fails
        await expect(asset.connect(signer_C).redeemFrom(signer_E.address, AMOUNT, DATA)).to.be.rejectedWith(
          "AccountIsBlocked",
        );
    ...
    describe("Paused", () => {
      ...
      it("GIVEN a paused Token WHEN redeem THEN transaction fails with TokenIsPaused", async () => {
        // transfer with data fails
        await expect(asset.connect(signer_C).redeem(AMOUNT, DATA)).to.be.rejectedWith("TokenIsPaused");
    ...
    describe("AccessControl", () => {
      it("GIVEN an account without issuer role WHEN issue THEN transaction fails with AccountHasNoRole", async () => {
        // add to list fails
        await expect(asset.connect(signer_B).issue(signer_E.address, AMOUNT, DATA)).to.be.rejectedWith(
          "AccountHasNoRole",
        );
      });
    });
    ...
    describe("Kyc", () => {
      ...
      it(
        ...
        async () => {
          ...
          // non kyc'd sender
          await expect(
            asset.connect(signer_E).transferWithData(signer_D.address, AMOUNT, DATA),
          ).to.revertedWithCustomError(asset, "InvalidKycStatus");
          await expect(
            asset.connect(signer_B).transferFromWithData(signer_E.address, signer_A.address, AMOUNT, DATA),
          ).to.revertedWithCustomError(asset, "InvalidKycStatus");
          // non kyc'd receiver
          await expect(
            asset.connect(signer_D).transferWithData(signer_E.address, AMOUNT, DATA),
          ).to.revertedWithCustomError(asset, "InvalidKycStatus");
          await expect(
            asset.connect(signer_A).transferFromWithData(signer_D.address, signer_E.address, AMOUNT, DATA),
          ).to.revertedWithCustomError(asset, "InvalidKycStatus");
        },
      );
      it(
