...
describe("Cap Tests", () => {
  ...
  it("GIVEN an initialized contract WHEN trying to initialize it again THEN transaction fails with AlreadyInitialized", async () => {
    await expect(asset.initialize_Cap(5, [])).to.be.rejectedWith("AlreadyInitialized");
  });
  ...
  describe("Paused", () => {
    ...
    it("GIVEN a paused Token WHEN setMaxSupply THEN transaction fails with TokenIsPaused", async () => {
      ...
      await expect(asset.connect(signer_C).setMaxSupply(maxSupply)).to.be.rejectedWith("TokenIsPaused");
    });
  ...
  describe("AccessControl", () => {
    it("GIVEN an account without cap role WHEN setMaxSupply THEN transaction fails with AccountHasNoRole", async () => {
      // add to list fails
      await expect(asset.connect(signer_C).setMaxSupply(maxSupply)).to.be.rejectedWith(Error);
    });
  ...
  describe("Adjust balances", () => {
    ...
    it("GIVEN a token WHEN setMaxSupply THEN balance adjustments are included", async () => {
      ...
      await expect(asset.setMaxSupply(maxSupplyByPartition)).to.eventually.be.rejectedWith("NewMaxSupplyTooLow");
    });
    ...
    it("GIVEN a token WHEN max supply and partition max supply are set THEN balance adjustments occur and resetting partition max supply fails with NewMaxSupplyForPartitionTooLow", async () => {
      ...
      await expect(asset.setMaxSupplyByPartition(_PARTITION_ID_1, maxSupplyByPartition)).to.eventually.be.rejectedWith(
