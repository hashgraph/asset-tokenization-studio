...
import { ResolverProxy, CouponFixedRateFacetTimeTravel, FixedRate } from "@contract-types";
...
describe("Bond Fixed Rate Tests", () => {
  ...
  let couponFixedRateFacet: CouponFixedRateFacetTimeTravel;
  ...
  it("GIVEN a fixed rate bond WHEN setting a coupon with pending status THEN transaction success", async () => {
    ...
    const [registeredCoupon] = await couponFixedRateFacet.getCoupon(1);
