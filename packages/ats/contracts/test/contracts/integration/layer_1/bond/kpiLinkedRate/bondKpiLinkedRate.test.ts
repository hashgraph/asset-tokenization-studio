...
describe("Bond KpiLinked Rate Tests", () => {
  ...
  async function checkCouponPostValues(
    ...
    const [registeredCouponPostFixingDate] = await couponFacet.getCoupon(couponID);
    const couponForPostFixingDate = await couponFacet.getCouponFor(couponID, accountAddress);
    const couponAmountForPostFixingDate = await couponFacet.getCouponAmountFor(couponID, accountAddress);
  ...
  describe("KpiLinkedRateFacet", () => {
    it("GIVEN a kpiLinked rate bond WHEN setting a coupon with non pending status THEN transaction fails with InterestRateIsKpiLinked", async () => {
      ...
      await expect(couponFacet.setCoupon(couponData)).to.be.revertedWithCustomError(
        couponFacet,
    ...
    it("GIVEN a kpiLinked rate bond WHEN setting a coupon with pending status THEN transaction success", async () => {
      await expect(couponFacet.connect(signer_A).setCoupon(couponData))
    ...
    it("GIVEN a kpiLinked rate bond WHEN rate is during start Period THEN transaction success and rate is start rate", async () => {
      ...
      const [registeredCouponPreFixingDate] = await couponFacet.getCoupon(1);
      const couponForPreFixingDate = await couponFacet.getCouponFor(1, signer_A.address);
      const couponAmountForPreFixingDate = await couponFacet.getCouponAmountFor(1, signer_A.address);
  ...
  it("GIVEN a cancelled coupon WHEN triggerScheduledCrossOrderedTasks executes THEN coupon is not added in ordered list", async () => {
    ...
    await couponFacet.connect(signer_A).setCoupon(coupon1);
    await couponFacet.connect(signer_A).setCoupon(coupon2);

    await couponFacet.connect(signer_A).cancelCoupon(1);
    ...
    const orderedList = await couponFacet.getCouponsOrderedList(0, 10);
  ...
  describe("Bond Read - Ordered List", () => {
    ...
    describe("getCouponsOrderedListTotal", () => {
      it("should return 0 when no coupons have been created", async () => {
        const total = await couponFacet.getCouponsOrderedListTotal();
      ...
      it("should return the correct count when coupons exist", async () => {
        ...
        await couponFacet.setCoupon(coupon1);
        ...
        await couponFacet.setCoupon(coupon2);
        ...
        await couponFacet.setCoupon(coupon3);
    ...
    describe("getCouponFromOrderedListAt", () => {
      it("should return 0 for invalid position when no coupons exist", async () => {
        const couponId = await couponFacet.getCouponFromOrderedListAt(0);
      ...
      it("should return 0 for position beyond the list", async () => {
        ...
        await couponFacet.setCoupon(coupon);
        ...
        const couponId = await couponFacet.getCouponFromOrderedListAt(1);
        expect(couponId).to.equal(0);
      });
      ...
      it("should return correct coupon ID at specific position", async () => {
        ...
        const tx1 = await couponFacet.setCoupon(coupon1);
        await tx1.wait();
        ...
        const tx2 = await couponFacet.setCoupon(coupon2);
        await tx2.wait();
        ...
        const tx3 = await couponFacet.setCoupon(coupon3);
        await tx3.wait();
        ...
        const couponId0 = await couponFacet.getCouponFromOrderedListAt(0);
        expect(couponId0).to.equal(1);

        // Get coupon at position 1 (second coupon)
        const couponId1 = await couponFacet.getCouponFromOrderedListAt(1);
        expect(couponId1).to.equal(2);

        // Get coupon at position 2 (third coupon)
        const couponId2 = await couponFacet.getCouponFromOrderedListAt(2);
    ...
    describe("getCouponsOrderedList", () => {
      it("should return empty array when no coupons exist", async () => {
        const coupons = await couponFacet.getCouponsOrderedList(0, 10);
      ...
      it("should return paginated results correctly", async () => {
        ...
        const page1 = await couponFacet.getCouponsOrderedList(0, 2);
        expect(page1).to.be.an("array").with.lengthOf(2);
        expect(page1[0]).to.equal(1);
        expect(page1[1]).to.equal(2);

        // Get second page (2 items)
        const page2 = await couponFacet.getCouponsOrderedList(1, 2);
        ...
        const page3 = await couponFacet.getCouponsOrderedList(2, 2);
        expect(page3).to.be.an("array").with.lengthOf(1);
        expect(page3[0]).to.equal(5);

        // Get page beyond available data
        const page4 = await couponFacet.getCouponsOrderedList(3, 2);
