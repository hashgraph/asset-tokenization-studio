...
export function dividendTests(getCtx: () => AssetMockCtx): void {
  describe("Dividends", () => {
    ...
    it("GIVEN a dividend created WHEN calling dividend methods with a wrong dividendId THEN transactions fail with WrongIndexForAction", async () => {
      ...
      await expect(asset.connect(signer_C).cancelDividend(2)).to.be.revertedWithCustomError(
        asset,
        "WrongIndexForAction",
      );
    ...
    describe("Deactivated", () => {
      it("GIVEN a deactivated asset WHEN setDividend THEN transaction fails with Deactivated", async () => {
        ...
        await expect(
          asset.connect(signer_A).setDividend({ recordDate: 0, executionDate: 0, amount: 0, amountDecimals: 0 }),
      ...
      it("GIVEN a deactivated asset WHEN cancelDividend THEN transaction fails with Deactivated", async () => {
        await asset.forceDeactivate();
        await expect(asset.connect(signer_A).cancelDividend(0)).to.be.revertedWithCustomError(asset, "Deactivated");
      });

      it("GIVEN a deactivated asset WHEN forceCancelDividend THEN transaction fails with Deactivated", async () => {
        await asset.forceDeactivate();
        await expect(asset.connect(signer_A).forceCancelDividend(0)).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
