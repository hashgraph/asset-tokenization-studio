...
import { DEFAULT_PARTITION, ATS_ROLES, BOND_CONFIG_ID, TIME_PERIODS_S, ZERO, EMPTY_STRING } from "@scripts";
import { deployAtsInfrastructureFixture, getDltTimestamp, executeRbac } from "@test";
...
describe("CouponSecurityHolders Tests", () => {
  ...
  describe("initializeCouponSecurityHolders", () => {
    it("GIVEN a caller without DEFAULT_ADMIN_ROLE WHEN initializeCouponSecurityHolders is called THEN it reverts with AccountHasNoRole", async () => {
      ...
      await expect(freshAsset.connect(signer_B).initializeCouponSecurityHolders()).to.be.revertedWithCustomError(
        freshAsset,
        "AccountHasNoRole",
      );
    });
    ...
    it("GIVEN an already-initialised facet WHEN initializeCouponSecurityHolders is called again THEN it reverts with FacetAlreadyRegistered", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await freshAsset.connect(infra.deployer).initializeCouponSecurityHolders();
      await expect(freshAsset.connect(infra.deployer).initializeCouponSecurityHolders()).to.be.revertedWithCustomError(
        freshAsset,
    ...
    it("GIVEN a caller with DEFAULT_ADMIN_ROLE WHEN initializeCouponSecurityHolders is called THEN it emits CouponSecurityHoldersInitialized", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await expect(freshAsset.connect(infra.deployer).initializeCouponSecurityHolders()).to.emit(
        freshAsset,
