import { IAsset, MockDiamondCut, ResolverProxy } from "@contract-types";
import { HardhatEthersSigner } from "@nomicfoundation/hardhat-ethers/signers";
import { loadFixture } from "@nomicfoundation/hardhat-network-helpers";
import { ATS_ROLES, ZERO, DEFAULT_PARTITION, EMPTY_STRING, EQUITY_CONFIG_ID, RESOLVER_KEY_VOTING } from "@scripts";
import { deployEquityTokenFixture, executeRbac, MAX_UINT256, grantRoleAndPauseToken } from "@test";
...
describe("Voting rights", () => {
  ...
  describe("Deactivated", () => {
    it("GIVEN a deactivated asset WHEN setVoting THEN transaction fails with Deactivated", async () => {
      ...
      await expect(
        deactivatedAsset.connect(base.deployer).setVoting({ recordDate: 0, data: "0x" }),
