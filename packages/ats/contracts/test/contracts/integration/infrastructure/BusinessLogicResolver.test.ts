...
import { IAccessControl, IPause, BusinessLogicResolver } from "@contract-types";
...
describe("BusinessLogicResolver", () => {
  ...
  async function deployBusinessLogicResolverFixture() {
    ...
    accessControl = await ethers.getContractAt("IAccessControl", businessLogicResolver.target, signer_A);
    await accessControl.grantRole(ATS_ROLES._PAUSER_ROLE, signer_B.address);

    pause = await ethers.getContractAt("IPause", businessLogicResolver.target);
  }
