...
export function maturityTests(getCtx: () => AssetMockCtx): void {
  describe("Maturity Tests", () => {
    ...
    beforeEach(async () => {
      ...
      // The mega-mock starts in EVM-default state. Reproduce the legacy bond fixture's
      // role grants, issuer setup, and KYC. The legacy fixture also set the maturity date
      // at deploy time — set it here via the real role-gated setter.
      ...
      // Set a future maturity date matching the legacy fixture behaviour.
    ...
    describe("Deactivated", () => {
      ...
      it("GIVEN a deactivated asset WHEN fullRedeemAtMaturity THEN transaction fails with Deactivated", async () => {
        await expect(asset.connect(signer_A).fullRedeemAtMaturity(ethers.ZeroAddress)).to.be.revertedWithCustomError(
          asset,
          "Deactivated",
        );
      });
