...
import { ATS_ROLES, DEFAULT_PARTITION, EIP1066_CODES, EMPTY_STRING, EQUITY_CONFIG_ID, ZERO } from "@scripts";
...
describe("Transfer Facet Tests", () => {
  ...
  describe("initializeTransfer", () => {
    it("GIVEN a caller without DEFAULT_ADMIN_ROLE WHEN initializeTransfer is called THEN it reverts with AccountHasNoRole", async () => {
      ...
      await expect(freshAsset.connect(signer_C).initializeTransfer()).to.be.revertedWithCustomError(
        freshAsset,
        "AccountHasNoRole",
      );
    });
    ...
    it("GIVEN an already-initialised facet WHEN initializeTransfer is called again THEN it reverts with FacetAlreadyRegistered", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await freshAsset.connect(infra.deployer).initializeTransfer();
      await expect(freshAsset.connect(infra.deployer).initializeTransfer()).to.be.revertedWithCustomError(
        freshAsset,
    ...
    it("GIVEN a caller with DEFAULT_ADMIN_ROLE WHEN initializeTransfer is called THEN it emits TransferInitialized", async () => {
      ...
      const { proxyAddress } = await decodeEvent(infra.factory, "ProxyDeployed", proxyReceipt!);
      const freshAsset = await ethers.getContractAt("IAsset", proxyAddress as string);
      await expect(freshAsset.connect(infra.deployer).initializeTransfer()).to.emit(freshAsset, "TransferInitialized");
    });
  });
});
