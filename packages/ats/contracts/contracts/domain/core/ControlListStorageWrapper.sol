...
/// @title ControlListStorageWrapper — Control list (whitelist/blacklist) library
...
library ControlListStorageWrapper {
    ...
    function isExternallyAuthorized(address account) internal view returns (bool) {
        uint256 length = ExternalListsStorageWrapper.getExternalListsCount(_CONTROL_LIST_MANAGEMENT_STORAGE_POSITION);

        for (uint256 index; index < length; ) {
            address listAddr = ExternalListsStorageWrapper.getExternalListAt(
                _CONTROL_LIST_MANAGEMENT_STORAGE_POSITION,
                index
            );
