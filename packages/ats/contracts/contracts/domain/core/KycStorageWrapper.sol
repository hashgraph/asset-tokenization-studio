...
import { IRevocationList } from "../../facets/core/ssi/IRevocationList.sol";
import { IExternalKycList } from "../../facets/core/externalKycList/IExternalKycList.sol";
import { EnumerableSet } from "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import { Pagination } from "../../infrastructure/utils/Pagination.sol";
import { SSIStorageWrapper } from "./SSIStorageWrapper.sol";
...
/// @title KycStorageWrapper — Know Your Customer (KYC) management library
/// @notice Centralized KYC functionality including internal and external KYC verification
/// @dev Uses free function storage accessors from CoreStorage.sol and SSIStorageWrapper for SSI queries
library KycStorageWrapper {
    ...
    function getKycStatusFor(address account) internal view returns (IKyc.KycStatus) {
        ...
        if (!SSIStorageWrapper.isIssuer(kycFor.issuer)) return IKyc.KycStatus.NOT_GRANTED;

        address revocationListAddress = SSIStorageWrapper.getRevocationRegistryAddress();
    ...
    function isExternallyGranted(address account, IKyc.KycStatus status) internal view returns (bool) {
        ...
        for (uint256 index; index < length; ) {
            address listAddr = ExternalListsStorageWrapper.getExternalListAt(_KYC_MANAGEMENT_STORAGE_POSITION, index);
