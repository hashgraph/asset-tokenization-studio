...
import { ILock } from "../../facets/core/lock/ILock.sol";
...
library LockStorageWrapper {
    ...
    function updateTotalLock(bytes32 _partition, address _tokenHolder) internal returns (uint256 abaf_) {
        ...
        uint256 labaf = ABAFStorageWrapper.getTotalLockLabaf(_tokenHolder);
        uint256 labafByPartition = ABAFStorageWrapper.getTotalLockLabafByPartition(_partition, _tokenHolder);

        if (abaf_ != labaf) {
            uint256 factor = ABAFStorageWrapper.calculateFactor(abaf_, labaf);
            lockStorage().totalLockedAmountByAccount[_tokenHolder] *= factor;
            ABAFStorageWrapper.setTotalLockLabaf(_tokenHolder, abaf_);
        }
        ...
        if (abaf_ != labafByPartition) {
            ...
            ABAFStorageWrapper.setTotalLockLabafByPartition(_partition, _tokenHolder, abaf_);
        }
    }
    ...
    function updateLockByIndex(bytes32 _partition, uint256 _lockId, address _tokenHolder, uint256 _abaf) internal {
        uint256 lockLabaf = ABAFStorageWrapper.getLockLabafById(_partition, _tokenHolder, _lockId);
        if (_abaf != lockLabaf) {
            uint256 factorLock = ABAFStorageWrapper.calculateFactor(_abaf, lockLabaf);
    ...
    function getLockedAmountByPartitionAdjustedAt(
        ...
        uint256 factor = ABAFStorageWrapper.calculateFactor(
            ...
            ABAFStorageWrapper.getTotalLockLabafByPartition(_partition, _tokenHolder)
        );
    ...
    function getLockedAmountAdjustedAt(address _tokenHolder, uint256 _timestamp) internal view returns (uint256) {
        uint256 factor = ABAFStorageWrapper.calculateFactorForLockedAmountAdjustedAt(_tokenHolder, _timestamp);
    ...
    function getLockForByPartitionAdjustedAt(
        ...
        uint256 factor = ABAFStorageWrapper.calculateFactor(
            ABAFStorageWrapper.getAbafAdjustedAt(_timestamp),
            ABAFStorageWrapper.getLockLabafById(_partition, _tokenHolder, _lockId)
        );
