...
import {
    ...
    PartitionSnapshots,
    ListOfPartitions
} from "../../facets/core/snapshot/ISnapshots.sol";
import { ISnapshots } from "../../facets/core/snapshot/ISnapshots.sol";
import { ABAFStorageWrapper } from "./ABAFStorageWrapper.sol";
...
library SnapshotsStorageWrapper {
    ...
    function updateAbafSnapshot() internal {
        updateSnapshot(snapshotStorage().abafSnapshots, ABAFStorageWrapper.getAbaf());
    }

    /// @notice Updates decimals snapshot
    function updateDecimalsSnapshot() internal {
        updateSnapshot(snapshotStorage().decimals, ERC20StorageWrapper.getDecimals());
    }

    /// @notice Updates total supply snapshot
    function updateAssetTotalSupplySnapshot() internal {
        updateSnapshot(snapshotStorage().totalSupplySnapshots, ERC1410StorageWrapper.totalSupply());
    }
    ...
    function updateAccountSnapshot(address account, bytes32 partition) internal {
        ...
        uint256 abafNow = ABAFStorageWrapper.getAbafAdjustedAt(block.timestamp);

        SnapshotStorage storage ss = snapshotStorage();

        if (abafNow == abafAtSnap) {
            // No ABAF adjustment needed - use raw balances
            updateSnapshot(ss.accountBalanceSnapshots[account], ERC1410StorageWrapper.balanceOf(account));
        ...
        uint256 balance = ABAFStorageWrapper.balanceOfAdjustedAt(account, block.timestamp);
        uint256 balanceForPartition = ABAFStorageWrapper.balanceOfByPartitionAdjustedAt(
            partition,
            account,
            block.timestamp
        );
    ...
    function updateAccountLockedBalancesSnapshot(address account, bytes32 partition) internal {
        SnapshotStorage storage ss = snapshotStorage();
        updateSnapshot(ss.accountLockedBalanceSnapshots[account], LockStorageWrapper.getLockedAmountFor(account));
        updateSnapshot(
    ...
    function updateAccountHeldBalancesSnapshot(address account, bytes32 partition) internal {
        SnapshotStorage storage ss = snapshotStorage();
        updateSnapshot(ss.accountHeldBalanceSnapshots[account], HoldStorageWrapper.getHeldAmountFor(account));
        updateSnapshot(
    ...
    function updateAccountFrozenBalancesSnapshot(address account, bytes32 partition) internal {
        SnapshotStorage storage ss = snapshotStorage();
        updateSnapshot(ss.accountFrozenBalanceSnapshots[account], ComplianceStorageWrapper.getFrozenTokens(account));
        updateSnapshot(
            ss.accountPartitionFrozenBalanceSnapshots[account][partition],
            ComplianceStorageWrapper.getFrozenTokensByPartition(account, partition)
        );
    }
    ...
    function updateAccountClearedBalancesSnapshot(address account, bytes32 partition) internal {
        SnapshotStorage storage ss = snapshotStorage();
        updateSnapshot(ss.accountClearedBalanceSnapshots[account], ClearingStorageWrapper.getClearedAmount(account));
        updateSnapshot(
            ss.accountPartitionClearedBalanceSnapshots[account][partition],
            ClearingStorageWrapper.getClearedAmountByPartition(partition, account)
        );
    }
    ...
    function updateTotalSupplySnapshot(bytes32 partition) internal {
        SnapshotStorage storage ss = snapshotStorage();
        updateSnapshot(ss.totalSupplySnapshots, ERC1410StorageWrapper.totalSupply());
        updateSnapshot(
    ...
    function abafAtSnapshot(uint256 snapshotId) internal view returns (uint256 abaf_) {
        ...
        return snapshotted ? value : ABAFStorageWrapper.getAbafAdjustedAt(block.timestamp);
    }
    ...
    function balanceOfAtSnapshot(uint256 _snapshotId, address _tokenHolder) internal view returns (uint256) {
        return
            _balanceOfAtAdjusted(
                ...
                ABAFStorageWrapper.balanceOfAdjustedAt(_tokenHolder, block.timestamp)
            );
    }
    ...
    function tokenHoldersAt(
        ...
        holders_ = new address[](Pagination.getSize(start, end, totalTokenHoldersAt(_snapshotId)));
    ...
    function _balanceOfAtAdjusted(
        ...
        uint256 abafCurrent = ABAFStorageWrapper.getAbafAdjustedAt(block.timestamp);
