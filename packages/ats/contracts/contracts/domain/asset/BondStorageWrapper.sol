...
import { IBondRead } from "../../facets/asset/bond/IBondRead.sol";
import { IBond } from "../../facets/asset/bond/IBond.sol";
...
library BondStorageWrapper {
    ...
    function initCoupon(bytes32 _actionId, IBondRead.Coupon memory _coupon) internal {
        ...
        ScheduledTasksStorageWrapper.addScheduledCrossOrderedTask(_coupon.recordDate, abi.encode(SNAPSHOT_TASK_TYPE));
        ScheduledTasksStorageWrapper.addScheduledSnapshot(_coupon.recordDate, abi.encode(_actionId));
        ScheduledTasksStorageWrapper.addScheduledCrossOrderedTask(
            _coupon.fixingDate,
            abi.encode(COUPON_LISTING_TASK_TYPE)
        );
        ScheduledTasksStorageWrapper.addScheduledCouponListing(_coupon.fixingDate, abi.encode(_actionId));
    }
    ...
    function updateCouponRate(
        ...
        CorporateActionsStorageWrapper.updateCorporateActionData(actionId, abi.encode(_coupon));
    }
    ...
    function getCouponsOrderedList(
        ...
        couponIDs_ = new uint256[](Pagination.getSize(start, end, getCouponsOrderedListTotalAdjustedAt(_timestamp)));
    ...
    function getCouponsOrderedListTotalAdjustedAt(uint256 _timestamp) internal view returns (uint256 total_) {
        return
            ...
            ScheduledTasksStorageWrapper.getPendingScheduledCouponListingTotalAt(_timestamp);
    }
    ...
    // composition (SnapshotsStorageWrapper + ERC1410StorageWrapper) and are composed at the facet level.
