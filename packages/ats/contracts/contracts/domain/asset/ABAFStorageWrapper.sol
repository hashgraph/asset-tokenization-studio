...
import { IAdjustBalances } from "../../facets/asset/adjustBalances/IAdjustBalances.sol";
...
library ABAFStorageWrapper {
    ...
    function getLabafByUserAndPartition(bytes32 partition, address account) internal view returns (uint256) {
        uint256 partitionsIndex = ERC1410StorageWrapper.getPartitionIndex(account, partition);
    ...
    function adjustTotalAndMaxSupplyForPartition(bytes32 partition) internal {
        ...
        ERC1410StorageWrapper.adjustTotalSupplyByPartition(partition, factor);
        CapStorageWrapper.adjustMaxSupplyByPartition(partition, factor);
    ...
    function adjustTotalBalanceAndPartitionBalanceFor(bytes32 partition, address account) internal {
        ...
        if (partitionsIndex != 0) {
            ...
            ERC1410StorageWrapper.multiplyPartitionAmount(account, partitionsIndex, partitionFactor);
        ...
        ERC1410StorageWrapper.multiplyBalance(account, factor);
    ...
    function balanceOfAdjustedAt(address account, uint256 timestamp) internal view returns (uint256) {
        ...
        return ERC1410StorageWrapper.balanceOf(account) * factor;
    }
    ...
    function balanceOfByPartitionAdjustedAt(
        ...
        uint256 partitionIndex = ERC1410StorageWrapper.getPartitionIndex(account, partition);
        if (partitionIndex == 0) return 0;
        return ERC1410StorageWrapper.getPartitionAmount(account, partitionIndex) * factor;
    }
    ...
    function totalSupplyByPartitionAdjustedAt(bytes32 partition, uint256 timestamp) internal view returns (uint256) {
        ...
        return ERC1410StorageWrapper.totalSupplyByPartition(partition) * factor;
    }
    ...
    /// Must be used instead of ERC1410StorageWrapper.addPartitionTo when ABAF is active
    function addPartitionToWithLabaf(uint256 value, address account, bytes32 partition) internal {
        ...
        ERC1410StorageWrapper.pushPartition(account, value, partition);
    }
