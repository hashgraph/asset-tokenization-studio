...
import { AccessStorageWrapper } from "../core/AccessStorageWrapper.sol";
...
library ERC1594StorageWrapper {
    ...
    function isAbleToTransferFromByPartition(
        ...
        if (_from == ZERO_ADDRESS || _to == ZERO_ADDRESS) {
            return (
                ...
                ERC1410StorageWrapper.ZeroAddressNotAllowed.selector,
                EMPTY_BYTES
            );
        }

        bool checkSender = _from != _sender &&
            !AccessStorageWrapper.hasRole(
        ...
        bool checkAllowance = checkSender && !ERC1410StorageWrapper.isAuthorized(_partition, _sender, _from);
    ...
    function _isCompliant(
        ...
        address complianceAddr = address(ComplianceStorageWrapper.getCompliance());

        if (_checkSender) {
            if (!ControlListStorageWrapper.isAbleToAccess(_sender)) {
                return (
                    false,
            ...
            if (ComplianceStorageWrapper.isRecovered(_sender)) {
                return (
                    false,
        ...
        if (_from != address(0)) {
            if (ComplianceStorageWrapper.isRecovered(_from)) {
                return (
                    false,
                    Eip1066.REVOKED_OR_BANNED,
                    IERC3643Management.WalletRecovered.selector,
            ...
            if (!ControlListStorageWrapper.isAbleToAccess(_from)) {
                return (
                    false,
                    Eip1066.DISALLOWED_OR_STOP,
                    IControlListBase.AccountIsBlocked.selector,
    ...
    function _isIdentified(
        ...
        address identityRegistryAddr = address(ComplianceStorageWrapper.getIdentityRegistry());

        if (_from != address(0)) {
            if (!KycStorageWrapper.verifyKycStatus(IKyc.KycStatus.GRANTED, _from)) {
        ...
        if (_to != address(0)) {
            if (!KycStorageWrapper.verifyKycStatus(IKyc.KycStatus.GRANTED, _to)) {
    ...
    function _businessLogicChecks(
        ...
        if (!ERC1410StorageWrapper.validPartition(_partition, _from)) {
            return (
                false,
                Eip1066.INSUFFICIENT_FUNDS,
        ...
        uint256 currentPartitionBalance = ABAFStorageWrapper.balanceOfByPartitionAdjustedAt(
            _partition,
            _from,
            _timestamp
        );
    ...
    function _allowanceAdjustedAt(address _owner, address _spender, uint256 _timestamp) private view returns (uint256) {
        uint256 factor = ABAFStorageWrapper.calculateFactor(
            ...
            ABAFStorageWrapper.getAllowanceLabaf(_owner, _spender)
        );
        return ERC20StorageWrapper.getAllowance(_owner, _spender) * factor;
    }
}
