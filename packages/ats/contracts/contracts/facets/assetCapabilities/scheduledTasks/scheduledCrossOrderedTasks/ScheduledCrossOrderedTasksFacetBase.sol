...
abstract contract ScheduledCrossOrderedTasksFacetBase is IScheduledCrossOrderedTasks, IStaticFunctionSelectors {
    // ════════════════════════════════════════════════════════════════════════════════════
    ...
    function _triggerBalanceAdjustments(uint256 _max) private {
        ...
        for (uint256 j = 1; j <= max; j++) {
            ...
            if (task.scheduledTimestamp < timestamp) {
                ...
                emit IAdjustBalancesStorageWrapper.AdjustmentBalanceSet(msg.sender, ba.factor, ba.decimals);
            } else {
                break;
            }
        }
