...
import { IExternalPauseManagement } from "../externalPause/IExternalPauseManagement.sol";
...
abstract contract ExternalPauseManagement is IExternalPauseManagement {
    ...
    function initialize_ExternalPauses(address[] calldata _pauses) external override {
        if (ExternalListsStorageWrapper.getExternalListsCount(_PAUSE_MANAGEMENT_STORAGE_POSITION) > 0) {
            revert AlreadyInitialized();
        }
        uint256 length = _pauses.length;
        for (uint256 index; index < length; ) {
            ExternalListsStorageWrapper.requireValidAddress(_pauses[index]);
            ExternalListsStorageWrapper.addExternalList(_PAUSE_MANAGEMENT_STORAGE_POSITION, _pauses[index]);
            unchecked {
                ++index;
            }
        }
        ExternalListsStorageWrapper.setExternalListInitialized(_PAUSE_MANAGEMENT_STORAGE_POSITION);
    }
    ...
    function updateExternalPauses(
        ...
        AccessStorageWrapper.checkRole(_PAUSE_MANAGER_ROLE);
    ...
    function addExternalPause(address _pause) external override returns (bool success_) {
        AccessStorageWrapper.checkRole(_PAUSE_MANAGER_ROLE);
        PauseStorageWrapper.requireNotPaused();
        ExternalListsStorageWrapper.requireValidAddress(_pause);
        success_ = ExternalListsStorageWrapper.addExternalList(_PAUSE_MANAGEMENT_STORAGE_POSITION, _pause);
        if (!success_) {
    ...
    function removeExternalPause(address _pause) external override returns (bool success_) {
        AccessStorageWrapper.checkRole(_PAUSE_MANAGER_ROLE);
        PauseStorageWrapper.requireNotPaused();
        success_ = ExternalListsStorageWrapper.removeExternalList(_PAUSE_MANAGEMENT_STORAGE_POSITION, _pause);
        if (!success_) {
    ...
    function isExternalPause(address _pause) external view override returns (bool) {
        return ExternalListsStorageWrapper.isExternalList(_PAUSE_MANAGEMENT_STORAGE_POSITION, _pause);
    }

    function getExternalPausesCount() external view override returns (uint256 externalPausesCount_) {
        return ExternalListsStorageWrapper.getExternalListsCount(_PAUSE_MANAGEMENT_STORAGE_POSITION);
    }
