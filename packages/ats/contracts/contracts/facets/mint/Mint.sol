...
abstract contract Mint is IMintFacet, Modifiers {
    ...
    /// @inheritdoc IMintFacet
    function isIssuable() external view override returns (bool issuable_) {
        issuable_ = ERC1594StorageWrapper.isIssuable();
    }

    /**
     * @notice Asserts that the current sender holds the issuer or agent role.
     * @dev Reverts via `AccessControlStorageWrapper.checkAnyRole` when neither role is present.
     */
    function _checkIssuerOrAgent() private view {
