 ...
 *      consistent with partition-scoped queries under test-mode overrides.
 */
abstract contract Hold is IHoldFacet, Modifiers {
    ...
    function getHeldAmountFor(address _tokenHolder) external view override returns (uint256 amount_) {
        return HoldStorageWrapper.getHeldAmountForAdjustedAt(_tokenHolder, EvmAccessors.getBlockTimestamp());
    }
