...
abstract contract ScheduledBalanceAdjustmentBase {
     ...
     * @param _newBalanceAdjustment The adjustment parameters supplied by the caller.
     ...
     * @param _balanceAdjustmentId One-based index of the balance adjustment to cancel.
     */
    function _cancelScheduledBalanceAdjustment(uint256 _balanceAdjustmentId) internal {
        unchecked {
        ...
        _requireNotExecuted(balanceAdjustment.executionDate, corporateActionId, _balanceAdjustmentId);
        CorporateActionsStorageWrapper.cancelCorporateAction(corporateActionId);
    }

    /**
     * @notice Registers a decoded balance adjustment with the scheduled-task sub-system.
     * @dev Reverts with `BalanceAdjustmentCreationFailed` when `_actionId` is zero,
     *      indicating the upstream corporate-action creation failed.
     * @param _actionId Corporate-action identifier assigned by the corporate-actions storage.
     * @param _data     ABI-encoded `ScheduledBalanceAdjustment` struct.
     */
    function _registerBalanceAdjustmentTasks(bytes32 _actionId, bytes memory _data) internal {
        _checkValidCorporateActionId(_actionId);
        ...
        ScheduledTasksStorageWrapper.addScheduledBalanceAdjustment(newBalanceAdjustment.executionDate, _actionId);
    }
     ...
     * @param _balanceAdjustmentId The identifier of the balance adjustment to cancel.
     */
    function _forceCancelScheduledBalanceAdjustment(uint256 _balanceAdjustmentId) internal {
        (, bytes32 corporateActionId, ) = _getScheduledBalanceAdjustment(_balanceAdjustmentId);
        CorporateActionsStorageWrapper.cancelCorporateAction(corporateActionId);
    }
     ...
     * @param _executionDate      Scheduled execution timestamp of the adjustment.
     * @param _corporateActionId  Corporate-action identifier, forwarded to the revert.
     * @param _balanceAdjustmentId One-based index, forwarded to the revert.
     */
    function _requireNotExecuted(
        ...
        if (_executionDate <= TimeTravelStorageWrapper.getBlockTimestamp()) {
            revert IScheduledBalanceAdjustment.BalanceAdjustmentAlreadyExecuted(
                _corporateActionId,
     ...
     * @param _balanceAdjustmentID One-based index of the balance adjustment to retrieve.
     ...
     * @param _actionId Corporate-action identifier to validate.
     */
    function _checkValidCorporateActionId(bytes32 _actionId) internal pure {
