...
abstract contract Freeze is IFreeze, TimestampProvider {
    function setAddressFrozen(address _userAddress, bool _freezStatus) external override {
        ...
        ERC1410StorageWrapper._requireValidAddress(_userAddress);
        {
        ...
        ERC3643StorageWrapper._setAddressFrozen(_userAddress, _freezStatus);
    ...
    function freezePartialTokens(address _userAddress, uint256 _amount) external override {
        PauseStorageWrapper._requireNotPaused();
        ERC3643StorageWrapper._requireUnrecoveredAddress(_userAddress);
        ERC1410StorageWrapper._requireValidAddress(_userAddress);
        ...
        ERC3643StorageWrapper._freezeTokens(_userAddress, _amount);
    ...
    function unfreezePartialTokens(address _userAddress, uint256 _amount) external override {
        ...
        ERC3643StorageWrapper._unfreezeTokens(_userAddress, _amount, _getBlockTimestamp());
        emit TokensUnfrozen(_userAddress, _amount, _DEFAULT_PARTITION);
    }

    function batchSetAddressFrozen(address[] calldata _userAddresses, bool[] calldata _freeze) external {
        PauseStorageWrapper._requireNotPaused();
        ERC3643StorageWrapper._requireValidInputBoolArrayLength(_userAddresses, _freeze);
        {
            bytes32[] memory roles = new bytes32[](2);
            roles[0] = _FREEZE_MANAGER_ROLE;
            roles[1] = _AGENT_ROLE;
        ...
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            ERC1410StorageWrapper._requireValidAddress(_userAddresses[i]);
            ERC3643StorageWrapper._setAddressFrozen(_userAddresses[i], _freeze[i]);
    ...
    function batchFreezePartialTokens(address[] calldata _userAddresses, uint256[] calldata _amounts) external {
        ...
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            ERC3643StorageWrapper._requireUnrecoveredAddress(_userAddresses[i]);
        }
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            ERC3643StorageWrapper._freezeTokens(_userAddresses[i], _amounts[i]);
    ...
    function batchUnfreezePartialTokens(address[] calldata _userAddresses, uint256[] calldata _amounts) external {
        ...
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            ERC3643StorageWrapper._unfreezeTokens(_userAddresses[i], _amounts[i], _getBlockTimestamp());
            emit TokensUnfrozen(_userAddresses[i], _amounts[i], _DEFAULT_PARTITION);
        }
    }

    function getFrozenTokens(address _userAddress) external view override returns (uint256) {
        return ERC3643StorageWrapper._getFrozenAmountForAdjustedAt(_userAddress, _getBlockTimestamp());
    }
}
