...
import { ROLE_DOCUMENTER } from "../../constants/roles.sol";
...
abstract contract Documentation is IDocumentation, Modifiers {
     ...
     *         existing one.
     * @dev Restricted to accounts holding `ROLE_DOCUMENTER` on an unpaused token.
    ...
    function removeDocument(
        bytes32 _name
    ) external override onlyActivated onlyUnpaused onlyRole(ROLE_DOCUMENTER) documentExists(_name) {
