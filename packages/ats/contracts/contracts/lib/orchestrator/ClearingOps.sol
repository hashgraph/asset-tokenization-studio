...
///      - Simple storage operations are done in facets or domain libs
library ClearingOps {
    ...
    // ==========================================================================
    // CLEARING CREATION OPERATIONS (Pure Orchestration)
    // ==========================================================================
    ...
    function clearingTransferCreation(
        ...
        if (_thirdPartyType == ThirdPartyType.NULL) {
            emit IClearingTransfer.ClearedTransferByPartition(msg.sender, _from, _to, _clearingOperation.partition, clearingId_, _amount, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else if (_thirdPartyType == ThirdPartyType.AUTHORIZED) {
            emit IClearingTransfer.ClearedTransferFromByPartition(msg.sender, _from, _to, _clearingOperation.partition, clearingId_, _amount, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else if (_thirdPartyType == ThirdPartyType.OPERATOR) {
            emit IClearingTransfer.ClearedOperatorTransferByPartition(msg.sender, _from, _to, _clearingOperation.partition, clearingId_, _amount, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else {
            emit IClearingTransfer.ProtectedClearedTransferByPartition(msg.sender, _from, _to, _clearingOperation.partition, clearingId_, _amount, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        }
        success_ = true;
    }
    ...
    function clearingRedeemCreation(
        ...
        if (_thirdPartyType == ThirdPartyType.NULL) {
            emit IClearingRedeem.ClearedRedeemByPartition(msg.sender, _from, _clearingOperation.partition, clearingId_, _amount, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else if (_thirdPartyType == ThirdPartyType.AUTHORIZED) {
            emit IClearingRedeem.ClearedRedeemFromByPartition(msg.sender, _from, _clearingOperation.partition, clearingId_, _amount, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else if (_thirdPartyType == ThirdPartyType.OPERATOR) {
            emit IClearingRedeem.ClearedOperatorRedeemByPartition(msg.sender, _from, _clearingOperation.partition, clearingId_, _amount, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else {
            emit IClearingRedeem.ProtectedClearedRedeemByPartition(msg.sender, _from, _clearingOperation.partition, clearingId_, _amount, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        }
        success_ = true;
    }
    ...
    function clearingHoldCreationCreation(
        ...
        if (_thirdPartyType == ThirdPartyType.NULL) {
            emit IClearingHoldCreation.ClearedHoldByPartition(msg.sender, _from, _clearingOperation.partition, clearingId_, _hold, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else if (_thirdPartyType == ThirdPartyType.AUTHORIZED) {
            emit IClearingHoldCreation.ClearedHoldFromByPartition(msg.sender, _from, _clearingOperation.partition, clearingId_, _hold, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else if (_thirdPartyType == ThirdPartyType.OPERATOR) {
            emit IClearingHoldCreation.ClearedOperatorHoldByPartition(msg.sender, _from, _clearingOperation.partition, clearingId_, _hold, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        } else {
            emit IClearingHoldCreation.ProtectedClearedHoldByPartition(msg.sender, _from, _clearingOperation.partition, clearingId_, _hold, _clearingOperation.expirationTimestamp, _clearingOperation.data, _operatorData);
        }
        success_ = true;
    }

    // ==========================================================================
    // CLEARING ACTION OPERATIONS (Pure Orchestration)
    // ==========================================================================
    ...
    function protectedClearingTransferByPartition(
        ...
        return clearingTransferCreation(
    ...
    function protectedClearingCreateHoldByPartition(
        ...
        LibNonce.setNonceFor(_protectedClearingOperation.nonce, _protectedClearingOperation.from);
        return clearingHoldCreationCreation(
            _protectedClearingOperation.clearingOperation,
            _protectedClearingOperation.from,
            _hold,
            "",
    ...
    // ==========================================================================
    // PRIVATE HELPERS (Orchestration Logic Only)
    // ==========================================================================
    ...
    ) private returns (bool success_, uint256 amount_, ThirdPartyType operatorType_, bytes memory operationData_, bytes32 partition_) {
    ...
    function _clearingTransferExecution(
        ...
        if (
            _tokenHolder != destination && address(LibCompliance.getCompliance()) != address(0) && _partition == DEFAULT_PARTITION
        ) {
    ...
    /// @notice Get clearing amount from identifier
