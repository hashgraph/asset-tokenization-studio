 ...
 *
 * This facet implements a `transfer` function with the same selector as TransferFacet.transfer (0xa9059cbb).
...
contract DuplicateSelectorFacetTest is IStaticFunctionSelectors {
    ...
    /**
     * @dev Intentionally uses the same selector as TransferFacet.transfer (0xa9059cbb).
     ...
     * Note: Only includes the transfer selector which conflicts with TransferFacet.
     */
    function getStaticFunctionSelectors() external pure override returns (bytes4[] memory staticFunctionSelectors_) {
        staticFunctionSelectors_ = new bytes4[](1);
        staticFunctionSelectors_[0] = this.transfer.selector; // 0xa9059cbb - duplicates TransferFacet.transfer
    }
