...
abstract contract Internals_OLD is Modifiers {
    // ===== Balance & Supply Query Methods =====
    ...
    // ===== Transfer & Redeem Validation Methods =====
    ...
    function _getERC20MetadataAdjustedAt(
        uint256 _timestamp
    ) internal view virtual returns (IERC20.ERC20Metadata memory);
    ...
    // ===== SSI Methods =====
    function _getRevocationRegistryAddress() internal view virtual returns (address);
    function _isIssuer(address _issuer) internal view virtual returns (bool);

    // ===== Snapshot Query Methods (Optional) =====
    ...
    function _updateTotalCleared(bytes32 _partition, address _account) internal virtual returns (uint256 abaf_);
    ...
    function _buildClearingOperationBasicInfo(
        IClearing.ClearingOperationIdentifier memory _clearingOperationIdentifier
    ) internal view virtual returns (IClearing.ClearingOperationBasicInfo memory);
    ...
    function _addressValueAt(
        PartitionSnapshots storage snapshots,
        address _account,
        uint256 snapshotId
    ...
    function _initCoupon(
        ...
        IBondRead.Coupon calldata _coupon,
        bool _autoCalling,
        uint256[] calldata _couponTimestamps
    ) internal virtual;
