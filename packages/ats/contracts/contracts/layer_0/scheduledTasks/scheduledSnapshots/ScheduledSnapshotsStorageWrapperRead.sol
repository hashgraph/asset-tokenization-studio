...
import { ScheduledTasksStorageWrapper } from '../scheduledTasks/ScheduledTasksStorageWrapper.sol';
import { _SCHEDULED_SNAPSHOTS_STORAGE_POSITION } from '../../constants/storagePositions.sol';

abstract contract ScheduledSnapshotsStorageWrapperRead is ScheduledTasksStorageWrapper {
    ...
    function _getScheduledSnapshots(
        uint256 _pageIndex,
        uint256 _pageLength
    ) internal view returns (ScheduledTasksLib.ScheduledTask[] memory scheduledSnapshot_) {
    ...
    function _scheduledSnapshotStorage()
        internal
        pure
        returns (ScheduledTasksLib.ScheduledTasksDataStorage storage scheduledSnapshots_)
    {
