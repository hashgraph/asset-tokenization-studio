...
import { ScheduledSnapshotsStorageWrapperRead } from '../scheduledSnapshots/ScheduledSnapshotsStorageWrapperRead.sol';
...
abstract contract ScheduledBalanceAdjustmentsStorageWrapper is ScheduledSnapshotsStorageWrapperRead {
    ...
    function _triggerScheduledBalanceAdjustments(uint256 _max) internal returns (uint256) {
        return
            ScheduledTasksLib.triggerScheduledTasks(
                ...
                IScheduledBalanceAdjustments.onScheduledBalanceAdjustmentTriggered.selector,
                _max,
                _blockTimestamp()
            );
    }
    ...
    function _getScheduledBalanceAdjustments(
        uint256 _pageIndex,
        uint256 _pageLength
    ) internal view returns (ScheduledTasksLib.ScheduledTask[] memory scheduledBalanceAdjustment_) {
    ...
    function _scheduledBalanceAdjustmentStorage()
        internal
        pure
        returns (ScheduledTasksLib.ScheduledTasksDataStorage storage scheduledBalanceAdjustments_)
    {
