    ...
    PauseStorageWrapper
{
    ...
    function _calculateInterestRateKpi(
        ...
        if (timestamp < params.interestRateData.startPeriod)
        ...
        int256 impactDelta = int256(impactData) -
            int256(params.impactLimits.baseLine);

        uint256 targetRate = params.interestRateData.limits.base;

        if (impactDelta >= 0) {
            uint256 impactDeltaRate = (uint256(impactDelta) *
                _DECIMALS_PRECISION) /
                (params.impactLimits.maxDeviationCap -
                    params.impactLimits.baseLine);
            targetRate =
                params.interestRateData.limits.base -
                (((params.interestRateData.limits.base -
                    params.interestRateData.limits.marginFloor) *
                    impactDeltaRate) / _DECIMALS_PRECISION);
        } else {
            uint256 impactDeltaRate = (uint256(-impactDelta) *
                _DECIMALS_PRECISION) /
                (params.impactLimits.baseLine -
                    params.impactLimits.maxDeviationFloor);
            targetRate =
                params.interestRateData.limits.base +
                (((params.interestRateData.limits.marginCap -
                    params.interestRateData.limits.base) * impactDeltaRate) /
                    _DECIMALS_PRECISION);
        }

        if (targetRate > params.interestRateData.limits.marginCap) {
            targetRate = params.interestRateData.limits.marginCap;
        }
    ...
    function _timeElapsed() internal view returns (uint256 seconds_) {
        return
            _blockTimestamp() - _interestRateKpiStorage().lastReportTimestamp;
    }

    function _interestRateKpiStorage()
        internal
        pure
        returns (InterestRateKpiDataStorage storage interestRateKpiData_)
    {
        bytes32 position = _INTEREST_RATE_KPI;
        // solhint-disable-next-line no-inline-assembly
        assembly {
            interestRateKpiData_.slot := position
        }
    }
}
