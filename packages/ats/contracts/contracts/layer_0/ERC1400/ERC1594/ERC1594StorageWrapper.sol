...
import { ZERO_ADDRESS, EMPTY_BYTES, _DEFAULT_PARTITION } from '../../../layer_0/constants/values.sol';
    ...
    function _isCompliant(
        ...
        if (_from != address(0)) {
            if (_isRecovered(_from)) {
                return (false, Eip1066.REVOKED_OR_BANNED, IERC3643Basic.WalletRecovered.selector, abi.encode(_from));
            }
        ...
        if (_to != address(0)) {
            if (_isRecovered(_to)) {
                return (false, Eip1066.REVOKED_OR_BANNED, IERC3643Basic.WalletRecovered.selector, abi.encode(_to));
            }
    ...
    function _isIdentified(
        ...
        if (_to != address(0)) {
            ...
            if (isVerifiedTo.length > 0 && !abi.decode(isVerifiedTo, (bool))) {
                return (false, Eip1066.DISALLOWED_OR_STOP, IERC3643Basic.AddressNotVerified.selector, abi.encode(_to));
            }
        }
