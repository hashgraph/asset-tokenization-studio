...
abstract contract CorporateActionsStorageWrapper1 is ClearingStorageWrapper1 {
    ...
    function _onScheduledSnapshotTriggered(uint256 _snapShotID, bytes memory _data) internal {
        if (_data.length > 0) {
    ...
    function _addSnapshotToAction(bytes32 _actionId, uint256 _snapshotId) internal {
        _updateCorporateActionResult(_actionId, SNAPSHOT_RESULT_ID, abi.encodePacked(_snapshotId));
    }
    ...
    function _getResult(bytes32 actionId, uint256 resultId) internal view returns (bytes memory result_) {
     ...
     */
    function _getCorporateActionResult(bytes32 actionId, uint256 resultId) internal view returns (bytes memory) {
        return _corporateActionsStorage().actionsData[actionId].results[resultId];
    }
