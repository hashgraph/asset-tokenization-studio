...
export const ConnectCommandFixture = createFixture<ConnectCommand>(
  (command) => {
    ...
    if (command.HWCSettings) {
      command.HWCSettings.as(() => HWCSettingsFixture.create());
    }
    if (command.custodialSettings) {
...
export const MirrorNodeFixture = createFixture<MirrorNode>((node) => {
  ...
  if (node.name) node.name.faker((faker) => faker.company.name());
  if (node.apiKey) node.apiKey.faker((faker) => faker.string.alphanumeric(32));
  if (node.headerName)
...
export const JsonRpcRelayFixture = createFixture<JsonRpcRelay>((relay) => {
  ...
  if (relay.name) relay.name.faker((faker) => faker.company.name());
  if (relay.apiKey)
