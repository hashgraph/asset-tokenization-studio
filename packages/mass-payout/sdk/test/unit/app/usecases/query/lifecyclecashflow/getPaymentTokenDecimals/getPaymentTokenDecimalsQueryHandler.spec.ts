...
describe("GetPaymentTokenDecimalsQueryHandler", () => {
  ...
  beforeEach(() => {
    ...
    handler = new GetPaymentTokenDecimalsQueryHandler(
      queryAdapter,
      contractService
    )
  })
  ...
  it("should return a GetPaymentTokenDecimalsQueryResponse on success", async () => {
    ...
    const mockEvmAddress = new EvmAddress(
    ...
    (queryAdapter.getPaymentTokenDecimals as jest.Mock).mockResolvedValueOnce(
      decimals
    )
    ...
    expect(contractService.getContractEvmAddress).toHaveBeenCalledWith(
      query.lifeCycleCashFlowId
    )
    expect(queryAdapter.getPaymentTokenDecimals).toHaveBeenCalledWith(
      mockEvmAddress
    )
  ...
  it("should throw GetPaymentTokenDecimalsQueryError if contractService fails", async () => {
    ...
    await expect(handler.execute(query)).rejects.toThrow(
      GetPaymentTokenDecimalsQueryError
    )
  })
  ...
  it("should throw GetPaymentTokenDecimalsQueryError if queryAdapter fails", async () => {
    ...
    queryAdapter.getPaymentTokenDecimals.mockRejectedValue(
      new Error("Error getting contract address")
    )
