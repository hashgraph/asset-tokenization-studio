...
describe("MirrorNodeAdapter", () => {
  ...
  describe("getAccountInfo", () => {
    ...
    it("should throw InvalidResponse on error", async () => {
      ...
      await expect(
        adapter.getAccountInfo(HederaIdPropsFixture.create().value)
  ...
  describe("getContractInfo", () => {
    ...
    it("should reject with InvalidResponse on error", async () => {
      mockedAxios.get.mockRejectedValueOnce(
        new Error("Error getting contract info")
      )

      await expect(
        adapter.getContractInfo(EvmAddressPropsFixture.create().value)
      ).rejects.toBeInstanceOf(InvalidResponse)
    })
  })
  ...
  describe("getTransactionResult", () => {
    it("should return transaction result", async () => {
      ...
      const result = await adapter.getTransactionResult(
    ...
    it("should throw InvalidResponse when no call_result", async () => {
      ...
      await expect(
        adapter.getTransactionResult(HederaIdPropsFixture.create().value)
      ).rejects.toBeInstanceOf(InvalidResponse)
    })
  })
  ...
  describe("getTransactionFinalError", () => {
    ...
    it("should throw TransactionNotFound if no transactions exist", async () => {
      ...
      await expect(
        adapter.getTransactionFinalError(HederaIdPropsFixture.create().value)
      ).rejects.toBeInstanceOf(InvalidResponse)
    }, 10000)
  })
  ...
  describe("accountToEvmAddress", () => {
    ...
    it("should throw ErrorRetrievingEvmAddress on error", async () => {
      jest
        .spyOn(adapter, "getAccountInfo")
        .mockRejectedValueOnce(new Error("fail"))

      await expect(
        adapter.accountToEvmAddress(HederaIdPropsFixture.create().value)
      ).rejects.toBeInstanceOf(ErrorRetrievingEvmAddress)
    })
  })
  ...
  describe("getHederaIdfromContractAddress", () => {
    it("should return id when contract address length >= 40", async () => {
      ...
      const contractEvmAddress = EvmAddressPropsFixture.create().value
      jest
        .spyOn(adapter, "getContractInfo")
        .mockResolvedValueOnce({
          id: contractId,
          evmAddress: contractEvmAddress,
        })

      const result = await adapter.getHederaIdfromContractAddress(
        "0x".padEnd(42, "1")
      )
