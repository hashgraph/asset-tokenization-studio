...
export class RPCQueryAdapter {
  ...
  async isPaused(lifeCycleCashFlow: EvmAddress): Promise<boolean> {
    this.logger.log(
      `Checking if the LifeCycleCashFlow contract ${lifeCycleCashFlow.toString()} is paused`
    )

    return await this.connect(
      LifeCycleCashFlow__factory,
      lifeCycleCashFlow.toString()
    ).isPaused()
  }

  async getPaymentToken(lifeCycleCashFlow: EvmAddress): Promise<string> {
    this.logger.log(
      `Getting the payment token for the contract ${lifeCycleCashFlow.toString()}`
    )
  ...
  async getPaymentTokenDecimals(
    ...
    this.logger.log(
      `Getting the payment token decimals for the contract ${lifeCycleCashFlow.toString()}`
    )
