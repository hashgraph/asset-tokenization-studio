...
describe('MintCommandHandler', () => {
    ...
    describe('execute', () => {
        it('should successfully mint tokens', async () => {
            ...
            expect(validationServiceMock.checkKycAddresses).toHaveBeenCalledWith(command.securityId, [command.targetId], KycStatus.GRANTED);
            expect(validationServiceMock.checkRole).toHaveBeenCalledWith(SecurityRole._ISSUER_ROLE, account.id.toString(), command.securityId);
