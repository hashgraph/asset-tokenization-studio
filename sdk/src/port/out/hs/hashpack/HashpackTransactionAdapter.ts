...
export class HashpackTransactionAdapter extends HederaTransactionAdapter {
  ...
  public async register(
    ...
    this.hashConnect = new HashConnect(
      LedgerId.fromString(this.networkService.environment),
    ...
    console.log('Hashpack init, events connected');
    //* Initialize
    ...
    if (
      ...
      this.setSigner();
      LogService.logTrace(
        `Previous pairing found for ${this.networkService.environment} with account ${this.account.id.toString()} and event: ${JSON.stringify(
          eventData,
        )}`,
      );
    }
    LogService.logTrace(
      `HashPack initialized for ${this.networkService.environment} with: ${JSON.stringify(
        eventData,
      )}`,
    );
    console.log(
