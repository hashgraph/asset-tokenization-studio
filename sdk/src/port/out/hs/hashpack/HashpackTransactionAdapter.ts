...
export class HashpackTransactionAdapter extends HederaTransactionAdapter {
  ...
  async init(network?: string): Promise<string> {
    ...
    if (this.initData.savedPairings.length > 0) {
      ...
      LogService.logTrace('Previous paring found: ', this.account, eventData);
    }
  ...
  async signAndSendTransaction(
    ...
    try {
      ...
      if (
        ...
        hashPackTransactionResponse = await t.executeWithSigner(this.signer);
        this.logTransaction(
          JSON.parse(
        ...
        this.logTransaction(
          hashPackTransactionResponse.response
            ? (hashPackTransactionResponse.response as any).transactionId ?? ''
            : (hashPackTransactionResponse.error as any).transactionId ?? '',
          this.networkService.environment,
        );
      }
  ...
  public setUpHashConnectEvents(): void {
    ...
    this.hc.connectionStatusChangeEvent.on((state) => {
      ...
      this.eventService.emit(WalletEvents.walletConnectionStatusChanged, {
        wallet: SupportedWallets.HASHPACK,
        status: this.hashConnectConectionState as unknown as ConnectionState,
      });
