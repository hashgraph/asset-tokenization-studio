...
interface IRole {
  ...
  grantRole(request: RoleRequest): Promise<boolean>;
  revokeRole(request: RoleRequest): Promise<boolean>;
  ...
  applyRoles(request: ApplyRolesRequest): Promise<boolean>;
}
...
class RoleInPort implements IRole {
  ...
  @LogError
  async grantRole(request: RoleRequest): Promise<boolean> {
    const { securityId, targetId, role } = request;
    handleValidation('RoleRequest', request);
  ...
  @LogError
  async revokeRole(request: RoleRequest): Promise<boolean> {
    const { securityId, targetId, role } = request;
    handleValidation('RoleRequest', request);
  ...
  @LogError
  async applyRoles(request: ApplyRolesRequest): Promise<boolean> {
