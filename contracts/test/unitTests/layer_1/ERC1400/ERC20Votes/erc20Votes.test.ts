...
describe('ERC20Votes Tests', () => {
    ...
    describe.skip('Delegation by Signature', () => {
        ...
        it('GIVEN a paused token WHEN delegateBySig THEN transaction fails with TokenIsPaused', async () => {
            // Pause the token
            await pauseFacet.pause()

            // Try to delegateBySig while paused
            await expect(
        ...
        it('GIVEN expired signature WHEN delegateBySig THEN reverts', async () => {
            const nonce = 1
        ...
        it('GIVEN reused nonce WHEN delegateBySig THEN reverts', async () => {
            ...
            await erc20VotesFacet.delegateBySig(
                account_B,
                nonce,
                expiry,
                sig.v,
            ...
            // Second delegation with same nonce fails
            await expect(
                erc20VotesFacet.delegateBySig(
                    account_B,
                    nonce,
                ...
                .to.be.revertedWithCustomError(erc20VotesFacet, 'WrongNounce')
