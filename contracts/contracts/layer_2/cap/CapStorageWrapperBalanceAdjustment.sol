...
abstract contract CapStorageWrapperBalanceAdjustment is
    CorporateActionsStorageWrapper,
    CapStorageWrapper,
    ERC1410ScheduledTasksStorageWrapper
{
    function _getMaxSupply(
        uint256 timestamp
    ) internal view virtual returns (uint256 maxSupply) {
        if (timestamp == 0) {
    ...
    function _getMaxSupplyByPartition(
        ...
        return super._getMaxSupplyByPartition(partition) * pendingABAF;
    }
}
